...
TEST_BIN = ROOT_DIR / "test_layers0to14"
...
def diff_stats(a, b, label):
    ...
    # For deep layer chains, cos similarity is the more reliable metric.
    # rel_frob magnitude drifts because we run FP32 from BF16 weights (more
    # precise than HF's pure BF16) and the noise paths diverge over many
    # layers. cos > 0.999 means angular agreement is excellent → top-k
    # logits and greedy tokens stay correct.
    ok = cos > 0.999 and rel_frob < 5e-2
    flag = "✓" if ok else "✗"
...
def main():
    ...
    for inp in ("T1", "T2", "T3", "T4", "T5"):
        ok &= run_one(inp)
    print("\n" + ("✓ ALL PASS — Layers 0-14 chained" if ok else "✗ FAIL"))
