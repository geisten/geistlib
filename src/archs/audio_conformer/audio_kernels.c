...
void layernorm_fp32_ws(
        size_t batch, size_t d, float eps, const float *x, const float gamma[static d], float *y) {
...
void glu_fp32(size_t batch, size_t d, const float *x, float *y) {
...
void depthwise_conv1d_causal_fp32(
        int channels, int t_in, int kernel, const float *in, const float *w, float *out) {
