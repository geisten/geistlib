...
static const char metal_q4k_gate_up_w4a8_source[] =
    ...
    "static inline float gelu(float x){return 0.5f*x*(1.0f+tanh(0.7978845608f*(x+0.044715f*x*x*x)));}\n"
...
static const char metal_q4k_gate_up_source[] =
    "#include <metal_stdlib>\n"
    "using namespace metal;\n"
    "struct P{uint n_in,n_out,rows,blocks_per_row,x_offset,gw,uw,gy,uy,xs,ys;};\n"
    "static inline float lf16(device const uchar*w,uint o){ushort b=ushort(w[o])|(ushort(w[o+1])<<8);return float(as_type<half>(b));}\n"
    "static inline void sm(uint j,device const uchar*q,thread uint&s,thread uint&m){if(j<4){s=uint(q[j])&63u;m=uint(q[j+4])&63u;}else{uint a=uint(q[j+4]),b=uint(q[j-4]),c=uint(q[j]);s=(a&15u)|((b>>6u)<<4u);m=(a>>4u)|((c>>6u)<<4u);}}\n"
    "static inline float dq(device const uchar*w,uint off,uint bpr,uint row,uint k){uint br=k/256u,ib=k-br*256u,sub=ib/32u,idx=ib-sub*32u;uint bo=off+(row*bpr+br)*144u;float d=lf16(w,bo),dm=lf16(w,bo+2u);uint s,m;sm(sub,w+bo+4u,s,m);uint qb=uint(w[bo+16u+(sub/2u)*32u+idx]);uint q=(sub&1u)==0u?(qb&15u):(qb>>4u);return d*float(s)*float(q)-dm*float(m);}\n"
    "static inline float gelu(float x){return 0.5f*x*(1.0f+tanh(0.7978845608028654f*(x+0.044715f*x*x*x)));}\n"
    "kernel void gate_up_q4k_m8(device const float*x[[buffer(0)]],device const uchar*gw[[buffer(1)]],device const uchar*uw[[buffer(2)]],device float*gy[[buffer(3)]],device float*uy[[buffer(4)]],constant P&p[[buffer(5)]],uint3 tg[[threadgroup_position_in_grid]],uint lid[[thread_index_in_threadgroup]]){(void)uy;threadgroup float gp[8][256];threadgroup float up[8][256];uint row=tg.x,bb=tg.y*8u;if(row>=p.n_out||bb>=p.rows){return;}float gs[8],us[8];for(uint m=0u;m<8u;m++){gs[m]=0.0f;us[m]=0.0f;}for(uint k=lid;k<p.n_in;k+=256u){float g=dq(gw,p.gw,p.blocks_per_row,row,k),u=dq(uw,p.uw,p.blocks_per_row,row,k);for(uint m=0u;m<8u;m++){uint b=bb+m;if(b<p.rows){float xv=x[p.x_offset+b*p.xs+k];gs[m]+=xv*g;us[m]+=xv*u;}}}for(uint m=0u;m<8u;m++){gp[m][lid]=gs[m];up[m][lid]=us[m];}threadgroup_barrier(mem_flags::mem_threadgroup);for(uint st=128u;st>0u;st>>=1u){if(lid<st){for(uint m=0u;m<8u;m++){gp[m][lid]+=gp[m][lid+st];up[m][lid]+=up[m][lid+st];}}threadgroup_barrier(mem_flags::mem_threadgroup);}if(lid==0u){for(uint m=0u;m<8u;m++){uint b=bb+m;if(b<p.rows){gy[p.gy+b*p.ys+row]=gelu(gp[m][0])*up[m][0];}}}}\n";
...
static const char metal_q4k_gate_up_n4_source[] =
    ...
    "kernel void gate_up_q4k_n4(device const float*x[[buffer(0)]],device const uchar*wg[[buffer(1)]],device const uchar*wu[[buffer(2)]],device float*gy[[buffer(3)]],device float*uy[[buffer(4)]],constant P&p[[buffer(5)]],uint3 tg[[threadgroup_position_in_grid]],ushort ti[[thread_index_in_simdgroup]],ushort sg[[simdgroup_index_in_threadgroup]]){(void)uy;uint ix=uint(ti)/8u,it=uint(ti)&7u,iq=it>>2u,ir=it&3u,b=tg.y,fr=(tg.x*2u+uint(sg))*2u;if(fr>=p.n_out||b>=p.rows)return;uint nb=p.n_in>>8u;float g0=0.0f,g1=0.0f,u0=0.0f,u1=0.0f;device const float*y4=x+p.xo+b*p.xs+ix*256u+64u*iq+8u*ir;for(uint ib=ix;ib<nb;ib+=4u){float yl[16],yh[16];float4 sumy=float4(0.0f);for(uint i=0u;i<8u;i++){yl[i]=y4[i];sumy[0]+=yl[i];yl[i+8u]=y4[i+32u];sumy[1]+=yl[i+8u];yh[i]=y4[i+128u];sumy[2]+=yh[i];yh[i+8u]=y4[i+160u];sumy[3]+=yh[i+8u];}if(fr<p.n_out){g0+=dotb(wg,p.gw,p.bpr,fr,ib,iq,ir,yl,yh,sumy);u0+=dotb(wu,p.uw,p.bpr,fr,ib,iq,ir,yl,yh,sumy);}if(fr+1u<p.n_out){g1+=dotb(wg,p.gw,p.bpr,fr+1u,ib,iq,ir,yl,yh,sumy);u1+=dotb(wu,p.uw,p.bpr,fr+1u,ib,iq,ir,yl,yh,sumy);}y4+=1024u;}float ag0=simd_sum(g0),au0=simd_sum(u0),ag1=simd_sum(g1),au1=simd_sum(u1);if(ti==0){uint go=p.gy+b*p.ys+fr;gy[go]=gelu(ag0)*au0;if(fr+1u<p.n_out)gy[go+1u]=gelu(ag1)*au1;}}\n";

static const char metal_q4k_gate_up_nt4_source[] =
    "#include <metal_stdlib>\n"
    "using namespace metal;\n"
    "struct P{uint n_in,n_out,rows,bpr,xo,gw,uw,gy,uy,xs,ys;};\n"
    "static inline uint u32(device const uchar*w,uint o){return uint(w[o])|(uint(w[o+1])<<8)|(uint(w[o+2])<<16)|(uint(w[o+3])<<24);}\n"
    "static inline float f32(device const uchar*w,uint o){return as_type<float>(u32(w,o));}\n"
    "static inline float dq(device const uchar*w,uint off,uint bpr,uint r,uint k){uint nt=r>>2u,nr=r&3u,br=k>>8u,ib=k&255u,sub=ib>>5u,bo=off+24u+((nt*bpr+br)*4u+nr)*280u;float d=f32(w,bo),dm=f32(w,bo+4u),sc=float(w[bo+8u+sub]),mn=float(w[bo+16u+sub]),q=float(w[bo+24u+ib]);return d*sc*q-dm*mn;}\n"
    "static inline float gelu(float x){return 0.5f*x*(1.0f+tanh(0.7978845608028654f*(x+0.044715f*x*x*x)));}\n"
    "kernel void gate_up_q4k_nt4(device const float*x[[buffer(0)]],device const uchar*wg[[buffer(1)]],device const uchar*wu[[buffer(2)]],device float*gy[[buffer(3)]],device float*uy[[buffer(4)]],constant P&p[[buffer(5)]],uint3 tg[[threadgroup_position_in_grid]],uint lid[[thread_index_in_threadgroup]]){(void)uy;threadgroup float g0[64],g1[64],g2[64],g3[64],u0[64],u1[64],u2[64],u3[64];uint r0=tg.x*4u,b=tg.y;if(r0>=p.n_out||b>=p.rows)return;bool h1=r0+1u<p.n_out,h2=r0+2u<p.n_out,h3=r0+3u<p.n_out;float gs0=0.0f,gs1=0.0f,gs2=0.0f,gs3=0.0f,us0=0.0f,us1=0.0f,us2=0.0f,us3=0.0f;for(uint k=lid;k<p.n_in;k+=64u){float xv=x[p.xo+b*p.xs+k];gs0+=xv*dq(wg,p.gw,p.bpr,r0,k);us0+=xv*dq(wu,p.uw,p.bpr,r0,k);if(h1){gs1+=xv*dq(wg,p.gw,p.bpr,r0+1u,k);us1+=xv*dq(wu,p.uw,p.bpr,r0+1u,k);}if(h2){gs2+=xv*dq(wg,p.gw,p.bpr,r0+2u,k);us2+=xv*dq(wu,p.uw,p.bpr,r0+2u,k);}if(h3){gs3+=xv*dq(wg,p.gw,p.bpr,r0+3u,k);us3+=xv*dq(wu,p.uw,p.bpr,r0+3u,k);}}g0[lid]=gs0;g1[lid]=gs1;g2[lid]=gs2;g3[lid]=gs3;u0[lid]=us0;u1[lid]=us1;u2[lid]=us2;u3[lid]=us3;threadgroup_barrier(mem_flags::mem_threadgroup);for(uint st=32u;st>0u;st>>=1u){if(lid<st){g0[lid]+=g0[lid+st];g1[lid]+=g1[lid+st];g2[lid]+=g2[lid+st];g3[lid]+=g3[lid+st];u0[lid]+=u0[lid+st];u1[lid]+=u1[lid+st];u2[lid]+=u2[lid+st];u3[lid]+=u3[lid+st];}threadgroup_barrier(mem_flags::mem_threadgroup);}if(lid==0u){uint go=p.gy+b*p.ys+r0;gy[go]=gelu(g0[0])*u0[0];if(h1)gy[go+1u]=gelu(g1[0])*u1[0];if(h2)gy[go+2u]=gelu(g2[0])*u2[0];if(h3)gy[go+3u]=gelu(g3[0])*u3[0];}}\n";

static const char metal_q4k_gate_up_nt8_source[] =
    "#include <metal_stdlib>\n"
    "using namespace metal;\n"
    "struct P{uint n_in,n_out,rows,bpr,xo,gw,uw,gy,uy,xs,ys;};\n"
    "static inline uint u32(device const uchar*w,uint o){return uint(w[o])|(uint(w[o+1])<<8)|(uint(w[o+2])<<16)|(uint(w[o+3])<<24);}\n"
    ...
    "static inline float dq(device const uchar*w,uint off,uint bpr,uint r,uint k){uint nt=r>>2u,nr=r&3u,br=k>>8u,ib=k&255u,sub=ib>>5u,bo=off+24u+((nt*bpr+br)*4u+nr)*280u;float d=f32(w,bo),dm=f32(w,bo+4u),sc=float(w[bo+8u+sub]),mn=float(w[bo+16u+sub]),q=float(w[bo+24u+ib]);return d*sc*q-dm*mn;}\n"
    "static inline float gelu(float x){return 0.5f*x*(1.0f+tanh(0.7978845608028654f*(x+0.044715f*x*x*x)));}\n"
    "kernel void gate_up_q4k_nt8(device const float*x[[buffer(0)]],device const uchar*wg[[buffer(1)]],device const uchar*wu[[buffer(2)]],device float*gy[[buffer(3)]],device float*uy[[buffer(4)]],constant P&p[[buffer(5)]],uint3 tg[[threadgroup_position_in_grid]],uint lid[[thread_index_in_threadgroup]],uint sg[[simdgroup_index_in_threadgroup]],uint sl[[thread_index_in_simdgroup]]){(void)uy;threadgroup float g0[4],g1[4],g2[4],g3[4],g4[4],g5[4],g6[4],g7[4],u0[4],u1[4],u2[4],u3[4],u4[4],u5[4],u6[4],u7[4];uint r0=tg.x*8u,b=tg.y;if(r0>=p.n_out||b>=p.rows)return;if(lid<4u){g0[lid]=0.0f;g1[lid]=0.0f;g2[lid]=0.0f;g3[lid]=0.0f;g4[lid]=0.0f;g5[lid]=0.0f;g6[lid]=0.0f;g7[lid]=0.0f;u0[lid]=0.0f;u1[lid]=0.0f;u2[lid]=0.0f;u3[lid]=0.0f;u4[lid]=0.0f;u5[lid]=0.0f;u6[lid]=0.0f;u7[lid]=0.0f;}threadgroup_barrier(mem_flags::mem_threadgroup);bool h1=r0+1u<p.n_out,h2=r0+2u<p.n_out,h3=r0+3u<p.n_out,h4=r0+4u<p.n_out,h5=r0+5u<p.n_out,h6=r0+6u<p.n_out,h7=r0+7u<p.n_out;float gs0=0.0f,gs1=0.0f,gs2=0.0f,gs3=0.0f,gs4=0.0f,gs5=0.0f,gs6=0.0f,gs7=0.0f,us0=0.0f,us1=0.0f,us2=0.0f,us3=0.0f,us4=0.0f,us5=0.0f,us6=0.0f,us7=0.0f;for(uint k=lid;k<p.n_in;k+=64u){float xv=x[p.xo+b*p.xs+k];gs0+=xv*dq(wg,p.gw,p.bpr,r0,k);us0+=xv*dq(wu,p.uw,p.bpr,r0,k);if(h1){gs1+=xv*dq(wg,p.gw,p.bpr,r0+1u,k);us1+=xv*dq(wu,p.uw,p.bpr,r0+1u,k);}if(h2){gs2+=xv*dq(wg,p.gw,p.bpr,r0+2u,k);us2+=xv*dq(wu,p.uw,p.bpr,r0+2u,k);}if(h3){gs3+=xv*dq(wg,p.gw,p.bpr,r0+3u,k);us3+=xv*dq(wu,p.uw,p.bpr,r0+3u,k);}if(h4){gs4+=xv*dq(wg,p.gw,p.bpr,r0+4u,k);us4+=xv*dq(wu,p.uw,p.bpr,r0+4u,k);}if(h5){gs5+=xv*dq(wg,p.gw,p.bpr,r0+5u,k);us5+=xv*dq(wu,p.uw,p.bpr,r0+5u,k);}if(h6){gs6+=xv*dq(wg,p.gw,p.bpr,r0+6u,k);us6+=xv*dq(wu,p.uw,p.bpr,r0+6u,k);}if(h7){gs7+=xv*dq(wg,p.gw,p.bpr,r0+7u,k);us7+=xv*dq(wu,p.uw,p.bpr,r0+7u,k);}}float tg0=simd_sum(gs0),tg1=simd_sum(gs1),tg2=simd_sum(gs2),tg3=simd_sum(gs3),tg4=simd_sum(gs4),tg5=simd_sum(gs5),tg6=simd_sum(gs6),tg7=simd_sum(gs7),tu0=simd_sum(us0),tu1=simd_sum(us1),tu2=simd_sum(us2),tu3=simd_sum(us3),tu4=simd_sum(us4),tu5=simd_sum(us5),tu6=simd_sum(us6),tu7=simd_sum(us7);if(sl==0u&&sg<4u){g0[sg]=tg0;g1[sg]=tg1;g2[sg]=tg2;g3[sg]=tg3;g4[sg]=tg4;g5[sg]=tg5;g6[sg]=tg6;g7[sg]=tg7;u0[sg]=tu0;u1[sg]=tu1;u2[sg]=tu2;u3[sg]=tu3;u4[sg]=tu4;u5[sg]=tu5;u6[sg]=tu6;u7[sg]=tu7;}threadgroup_barrier(mem_flags::mem_threadgroup);if(lid==0u){uint go=p.gy+b*p.ys+r0;gy[go]=gelu(g0[0]+g0[1]+g0[2]+g0[3])*(u0[0]+u0[1]+u0[2]+u0[3]);if(h1)gy[go+1u]=gelu(g1[0]+g1[1]+g1[2]+g1[3])*(u1[0]+u1[1]+u1[2]+u1[3]);if(h2)gy[go+2u]=gelu(g2[0]+g2[1]+g2[2]+g2[3])*(u2[0]+u2[1]+u2[2]+u2[3]);if(h3)gy[go+3u]=gelu(g3[0]+g3[1]+g3[2]+g3[3])*(u3[0]+u3[1]+u3[2]+u3[3]);if(h4)gy[go+4u]=gelu(g4[0]+g4[1]+g4[2]+g4[3])*(u4[0]+u4[1]+u4[2]+u4[3]);if(h5)gy[go+5u]=gelu(g5[0]+g5[1]+g5[2]+g5[3])*(u5[0]+u5[1]+u5[2]+u5[3]);if(h6)gy[go+6u]=gelu(g6[0]+g6[1]+g6[2]+g6[3])*(u6[0]+u6[1]+u6[2]+u6[3]);if(h7)gy[go+7u]=gelu(g7[0]+g7[1]+g7[2]+g7[3])*(u7[0]+u7[1]+u7[2]+u7[3]);}}\n";

static const char metal_q4k_qk_source[] =
    "#include <metal_stdlib>\n"
    "using namespace metal;\n"
    "struct P{uint n_in,q_out,k_out,rows,bpr,xo,qw,kw,qy,ky,xs,qys,kys;};\n"
    "static inline float lf16(device const uchar*w,uint o){ushort b=ushort(w[o])|(ushort(w[o+1])<<8);return float(as_type<half>(b));}\n"
    "static inline void sm(uint j,device const uchar*q,thread uint&s,thread uint&m){if(j<4){s=uint(q[j])&63u;m=uint(q[j+4])&63u;}else{uint a=uint(q[j+4]),b=uint(q[j-4]),c=uint(q[j]);s=(a&15u)|((b>>6u)<<4u);m=(a>>4u)|((c>>6u)<<4u);}}\n"
    "static inline float dq(device const uchar*w,uint off,uint bpr,uint row,uint k){uint br=k/256u,ib=k-br*256u,sub=ib/32u,idx=ib-sub*32u;uint bo=off+(row*bpr+br)*144u;float d=lf16(w,bo),dm=lf16(w,bo+2u);uint s,m;sm(sub,w+bo+4u,s,m);uint qb=uint(w[bo+16u+(sub/2u)*32u+idx]);uint q=(sub&1u)==0u?(qb&15u):(qb>>4u);return d*float(s)*float(q)-dm*float(m);}\n"
    "kernel void qk_q4k(device const float*x[[buffer(0)]],device const uchar*qw[[buffer(1)]],device const uchar*kw[[buffer(2)]],device float*qy[[buffer(3)]],device float*ky[[buffer(4)]],constant P&p[[buffer(5)]],uint3 tg[[threadgroup_position_in_grid]],uint lid[[thread_index_in_threadgroup]]){threadgroup float qp[256];threadgroup float kp[256];uint row=tg.x,b=tg.y;if(b>=p.rows||row>=max(p.q_out,p.k_out)){return;}float qs=0.0f,ks=0.0f;for(uint i=lid;i<p.n_in;i+=256u){float xv=x[p.xo+b*p.xs+i];if(row<p.q_out){qs+=xv*dq(qw,p.qw,p.bpr,row,i);}if(row<p.k_out){ks+=xv*dq(kw,p.kw,p.bpr,row,i);}}qp[lid]=qs;kp[lid]=ks;threadgroup_barrier(mem_flags::mem_threadgroup);for(uint st=128u;st>0u;st>>=1u){if(lid<st){qp[lid]+=qp[lid+st];kp[lid]+=kp[lid+st];}threadgroup_barrier(mem_flags::mem_threadgroup);}if(lid==0u){if(row<p.q_out){qy[p.qy+b*p.qys+row]=qp[0];}if(row<p.k_out){ky[p.ky+b*p.kys+row]=kp[0];}}}\n"
    "static inline uint u32(device const uchar*w,uint o){return uint(w[o])|(uint(w[o+1])<<8)|(uint(w[o+2])<<16)|(uint(w[o+3])<<24);}\n"
    "static inline float f32(device const uchar*w,uint o){return as_type<float>(u32(w,o));}\n"
    "static inline float dqt(device const uchar*w,uint off,uint bpr,uint r,uint k){uint nt=r>>2u,nr=r&3u,br=k>>8u,ib=k&255u,sub=ib>>5u,bo=off+24u+((nt*bpr+br)*4u+nr)*280u;float d=f32(w,bo),dm=f32(w,bo+4u),sc=float(w[bo+8u+sub]),mn=float(w[bo+16u+sub]),q=float(w[bo+24u+ib]);return d*sc*q-dm*mn;}\n"
    "kernel void qk_q4k_nt4(device const float*x[[buffer(0)]],device const uchar*qw[[buffer(1)]],device const uchar*kw[[buffer(2)]],device float*qy[[buffer(3)]],device float*ky[[buffer(4)]],constant P&p[[buffer(5)]],uint3 tg[[threadgroup_position_in_grid]],uint lid[[thread_index_in_threadgroup]]){threadgroup float q0[64],q1[64],q2[64],q3[64],k0[64],k1[64],k2[64],k3[64];uint r0=tg.x*4u,b=tg.y;if(b>=p.rows||r0>=max(p.q_out,p.k_out))return;bool qh0=r0<p.q_out,qh1=r0+1u<p.q_out,qh2=r0+2u<p.q_out,qh3=r0+3u<p.q_out,kh0=r0<p.k_out,kh1=r0+1u<p.k_out,kh2=r0+2u<p.k_out,kh3=r0+3u<p.k_out;float qs0=0.0f,qs1=0.0f,qs2=0.0f,qs3=0.0f,ks0=0.0f,ks1=0.0f,ks2=0.0f,ks3=0.0f;for(uint i=lid;i<p.n_in;i+=64u){float xv=x[p.xo+b*p.xs+i];if(qh0)qs0+=xv*dqt(qw,p.qw,p.bpr,r0,i);if(qh1)qs1+=xv*dqt(qw,p.qw,p.bpr,r0+1u,i);if(qh2)qs2+=xv*dqt(qw,p.qw,p.bpr,r0+2u,i);if(qh3)qs3+=xv*dqt(qw,p.qw,p.bpr,r0+3u,i);if(kh0)ks0+=xv*dqt(kw,p.kw,p.bpr,r0,i);if(kh1)ks1+=xv*dqt(kw,p.kw,p.bpr,r0+1u,i);if(kh2)ks2+=xv*dqt(kw,p.kw,p.bpr,r0+2u,i);if(kh3)ks3+=xv*dqt(kw,p.kw,p.bpr,r0+3u,i);}q0[lid]=qs0;q1[lid]=qs1;q2[lid]=qs2;q3[lid]=qs3;k0[lid]=ks0;k1[lid]=ks1;k2[lid]=ks2;k3[lid]=ks3;threadgroup_barrier(mem_flags::mem_threadgroup);for(uint st=32u;st>0u;st>>=1u){if(lid<st){q0[lid]+=q0[lid+st];q1[lid]+=q1[lid+st];q2[lid]+=q2[lid+st];q3[lid]+=q3[lid+st];k0[lid]+=k0[lid+st];k1[lid]+=k1[lid+st];k2[lid]+=k2[lid+st];k3[lid]+=k3[lid+st];}threadgroup_barrier(mem_flags::mem_threadgroup);}if(lid==0u){uint qo=p.qy+b*p.qys+r0,ko=p.ky+b*p.kys+r0;if(qh0)qy[qo]=q0[0];if(qh1)qy[qo+1u]=q1[0];if(qh2)qy[qo+2u]=q2[0];if(qh3)qy[qo+3u]=q3[0];if(kh0)ky[ko]=k0[0];if(kh1)ky[ko+1u]=k1[0];if(kh2)ky[ko+2u]=k2[0];if(kh3)ky[ko+3u]=k3[0];}}\n";
...
static const char metal_elem_source[] =
    ...
    "static inline float gelu_tanh_f(float x){return 0.5f*x*(1.0f+tanh(0.7978845608028654f*(x+0.044715f*x*x*x)));}\n"
...
[[nodiscard]] static enum geist_status metal_profile_reset(
    ...
    memset(st->profile, 0, sizeof(st->profile));
    return GEIST_OK;
}

[[nodiscard]] static enum geist_status metal_profile_dump(
    struct geist_backend *be) {

    if (be == nullptr || be->state == nullptr) {
        return GEIST_E_INVALID_ARG;
    }
    metal_profile_print_summary(be->state);
    return GEIST_OK;
}
...
static void metal_decode_replay_bind_attention(
    struct metal_decode_replay_attention *r) {

    r->block.residual = &r->residual;
    r->block.attn_norm_weight = &r->attn_norm_weight;
    r->block.q_proj_weight = &r->q_proj_weight;
    r->block.k_proj_weight = &r->k_proj_weight;
    r->block.v_proj_weight = &r->v_proj_weight;
    r->block.q_norm_weight = &r->q_norm_weight;
    r->block.k_norm_weight = &r->k_norm_weight;
    r->block.v_norm_weight = &r->v_norm_weight;
    r->block.cos = &r->cos;
    r->block.sin = &r->sin;
    r->block.k_cache = &r->k_cache;
    r->block.v_cache = &r->v_cache;
    r->block.o_proj_weight = &r->o_proj_weight;
    r->block.post_attn_norm_weight = &r->post_attn_norm_weight;
    r->block.normed_scratch = &r->normed_scratch;
    r->block.q_scratch = &r->q_scratch;
    r->block.k_scratch = &r->k_scratch;
    r->block.v_scratch = &r->v_scratch;
    r->block.attn_scratch = &r->attn_scratch;
    r->block.o_scratch = &r->o_scratch;
    r->block.post_attn_scratch = &r->post_attn_scratch;
    r->block.out = &r->out;
}

static void metal_decode_replay_bind_attention_query(
    struct metal_decode_replay_attention_query *r) {
...
static void metal_decode_replay_bind_ffn(
    struct metal_decode_replay_ffn *r) {

    r->block.residual = &r->residual;
    r->block.ffn_norm_weight = &r->ffn_norm_weight;
    r->block.gate_weight = &r->gate_weight;
    r->block.up_weight = &r->up_weight;
    r->block.down_weight = &r->down_weight;
    r->block.post_ffw_norm_weight = &r->post_ffw_norm_weight;
    r->block.pre_ff_scratch = &r->pre_ff_scratch;
    r->block.gate_scratch = &r->gate_scratch;
    r->block.up_scratch = &r->up_scratch;
    r->block.ffn_out_scratch = &r->ffn_out_scratch;
    r->block.post_ff_scratch = &r->post_ff_scratch;
    r->block.out = &r->out;
}

static void metal_decode_replay_bind_ple(
    struct metal_decode_replay_ple *r) {

    r->block.hidden = &r->hidden;
    r->block.per_layer_input = &r->per_layer_input;
    r->block.per_layer_gate_weight = &r->per_layer_gate_weight;
    r->block.per_layer_proj_weight = &r->per_layer_proj_weight;
    r->block.post_per_layer_norm_weight = &r->post_per_layer_norm_weight;
    r->block.gate_scratch = &r->gate_scratch;
    r->block.proj_scratch = &r->proj_scratch;
    r->block.out = &r->out;
}

static void metal_decode_replay_bind_greedy_head(
    struct metal_decode_replay_greedy_head *r) {

    r->head.hidden = &r->hidden;
    r->head.norm_weight = &r->norm_weight;
    r->head.lm_head_weight = &r->lm_head_weight;
    r->head.normed_scratch = &r->normed_scratch;
    r->head.logits = &r->logits;
}

static void metal_decode_replay_record_attention(
    struct metal_state *st,
    ...
    struct geist_buffer *q_qk_packed,
    struct geist_buffer *k_qk_packed,
    const struct metal_rows_params *norm_params,
    const struct metal_q4k_params *q_params,
    const struct metal_q4k_params *k_params,
    const struct metal_q4k_qk_params *qk_params,
    const struct metal_q4k_params *v_params,
    const struct metal_norm_rope_params *q_norm_rope_params,
    const struct metal_k_norm_rope_append_params *k_norm_rope_append_params,
    const struct metal_v_norm_append_params *v_norm_append_params,
    const struct metal_attention_params *attention_params,
    const struct metal_q4k_params *o_params,
    const struct metal_post_norm_params *post_params) {

    struct metal_decode_replay_op *op =
        metal_decode_replay_append(st, METAL_DECODE_REPLAY_ATTENTION);
    if (op == nullptr) { return; }
    struct metal_decode_replay_attention *r = &op->u.attention;
    r->block = *block;
    r->residual = *block->residual;
    r->attn_norm_weight = *block->attn_norm_weight;
    r->q_proj_weight = *block->q_proj_weight;
    r->k_proj_weight = *block->k_proj_weight;
    r->v_proj_weight = *block->v_proj_weight;
    r->q_norm_weight = *block->q_norm_weight;
    r->k_norm_weight = *block->k_norm_weight;
    r->v_norm_weight = *block->v_norm_weight;
    r->cos = *block->cos;
    r->sin = *block->sin;
    r->k_cache = *block->k_cache;
    r->v_cache = *block->v_cache;
    r->o_proj_weight = *block->o_proj_weight;
    r->post_attn_norm_weight = *block->post_attn_norm_weight;
    r->normed_scratch = *block->normed_scratch;
    r->q_scratch = *block->q_scratch;
    r->k_scratch = *block->k_scratch;
    r->v_scratch = *block->v_scratch;
    r->attn_scratch = *block->attn_scratch;
    r->o_scratch = *block->o_scratch;
    r->post_attn_scratch = *block->post_attn_scratch;
    r->out = *block->out;
    r->v_w_q6 = v_w_q6;
    r->use_fused_qk = use_fused_qk;
    r->use_fused_qk_nt4 = use_fused_qk_nt4;
    r->rope_uses_q_position = rope_uses_q_position;
    r->q_qk_packed = q_qk_packed;
    r->k_qk_packed = k_qk_packed;
    r->norm_params = (struct metal_decode_replay_rows_params){
        .rows = norm_params->rows,
        .cols = norm_params->cols,
        .x_offset = norm_params->x_offset,
        .w_offset = norm_params->w_offset,
        .y_offset = norm_params->y_offset,
        .x_row_stride = norm_params->x_row_stride,
        .y_row_stride = norm_params->y_row_stride,
        .eps = norm_params->eps,
    };
    r->q_params = (struct metal_decode_replay_q4k_params){
        .n_in = q_params->n_in,
        .n_out = q_params->n_out,
        .rows = q_params->rows,
        .blocks_per_row = q_params->blocks_per_row,
        .x_offset = q_params->x_offset,
        .w_byte_offset = q_params->w_byte_offset,
        .y_offset = q_params->y_offset,
        .x_row_stride = q_params->x_row_stride,
        .y_row_stride = q_params->y_row_stride,
    };
    r->k_params = (struct metal_decode_replay_q4k_params){
        .n_in = k_params->n_in,
        .n_out = k_params->n_out,
        .rows = k_params->rows,
        .blocks_per_row = k_params->blocks_per_row,
        .x_offset = k_params->x_offset,
        .w_byte_offset = k_params->w_byte_offset,
        .y_offset = k_params->y_offset,
        .x_row_stride = k_params->x_row_stride,
        .y_row_stride = k_params->y_row_stride,
    };
    r->qk_params = (struct metal_decode_replay_q4k_qk_params){
        .n_in = qk_params->n_in,
        .q_out = qk_params->q_out,
        .k_out = qk_params->k_out,
        .rows = qk_params->rows,
        .blocks_per_row = qk_params->blocks_per_row,
        .x_offset = qk_params->x_offset,
        .q_w_byte_offset = qk_params->q_w_byte_offset,
        .k_w_byte_offset = qk_params->k_w_byte_offset,
        .q_y_offset = qk_params->q_y_offset,
        .k_y_offset = qk_params->k_y_offset,
        .x_row_stride = qk_params->x_row_stride,
        .q_y_row_stride = qk_params->q_y_row_stride,
        .k_y_row_stride = qk_params->k_y_row_stride,
    };
    r->v_params = (struct metal_decode_replay_q4k_params){
        .n_in = v_params->n_in,
        .n_out = v_params->n_out,
        .rows = v_params->rows,
        .blocks_per_row = v_params->blocks_per_row,
        .x_offset = v_params->x_offset,
        .w_byte_offset = v_params->w_byte_offset,
        .y_offset = v_params->y_offset,
        .x_row_stride = v_params->x_row_stride,
        .y_row_stride = v_params->y_row_stride,
    };
    r->q_norm_rope_params = (struct metal_decode_replay_norm_rope_params){
        .rows = q_norm_rope_params->rows,
        .heads = q_norm_rope_params->heads,
        .head_dim = q_norm_rope_params->head_dim,
        .x_offset = q_norm_rope_params->x_offset,
        .w_offset = q_norm_rope_params->w_offset,
        .cos_offset = q_norm_rope_params->cos_offset,
        .sin_offset = q_norm_rope_params->sin_offset,
        .x_row_stride = q_norm_rope_params->x_row_stride,
        .rope_row_stride = q_norm_rope_params->rope_row_stride,
        .rope_row_offset = q_norm_rope_params->rope_row_offset,
        .eps = q_norm_rope_params->eps,
    };
    r->k_norm_rope_append_params =
        (struct metal_decode_replay_k_norm_rope_append_params){
            .rows = k_norm_rope_append_params->rows,
            .heads = k_norm_rope_append_params->heads,
            .head_dim = k_norm_rope_append_params->head_dim,
            .x_offset = k_norm_rope_append_params->x_offset,
            .w_offset = k_norm_rope_append_params->w_offset,
            .cos_offset = k_norm_rope_append_params->cos_offset,
            .sin_offset = k_norm_rope_append_params->sin_offset,
            .cache_offset = k_norm_rope_append_params->cache_offset,
            .x_row_stride = k_norm_rope_append_params->x_row_stride,
            .rope_row_stride = k_norm_rope_append_params->rope_row_stride,
            .rope_row_offset = k_norm_rope_append_params->rope_row_offset,
            .q_position = k_norm_rope_append_params->q_position,
            .eps = k_norm_rope_append_params->eps,
        };
    r->v_norm_append_params =
        (struct metal_decode_replay_v_norm_append_params){
            .rows = v_norm_append_params->rows,
            .heads = v_norm_append_params->heads,
            .head_dim = v_norm_append_params->head_dim,
            .x_offset = v_norm_append_params->x_offset,
            .w_offset = v_norm_append_params->w_offset,
            .cache_offset = v_norm_append_params->cache_offset,
            .x_row_stride = v_norm_append_params->x_row_stride,
            .q_position = v_norm_append_params->q_position,
            .eps = v_norm_append_params->eps,
        };
    r->attention_params = (struct metal_decode_replay_attention_params){
        .rows = attention_params->rows,
        .kv_len = attention_params->kv_len,
        .q_heads = attention_params->q_heads,
        .kv_heads = attention_params->kv_heads,
        .head_dim = attention_params->head_dim,
        .q_position = attention_params->q_position,
        .sliding_window = attention_params->sliding_window,
        .q_offset = attention_params->q_offset,
        .k_cache_offset = attention_params->k_cache_offset,
        .v_cache_offset = attention_params->v_cache_offset,
        .y_offset = attention_params->y_offset,
    };
    r->o_params = (struct metal_decode_replay_q4k_params){
        .n_in = o_params->n_in,
        .n_out = o_params->n_out,
        .rows = o_params->rows,
        .blocks_per_row = o_params->blocks_per_row,
        .x_offset = o_params->x_offset,
        .w_byte_offset = o_params->w_byte_offset,
        .y_offset = o_params->y_offset,
        .x_row_stride = o_params->x_row_stride,
        .y_row_stride = o_params->y_row_stride,
    };
    r->post_params = (struct metal_decode_replay_post_norm_params){
        .rows = post_params->rows,
        .cols = post_params->cols,
        .residual_offset = post_params->residual_offset,
        .x_offset = post_params->x_offset,
        .w_offset = post_params->w_offset,
        .y_offset = post_params->y_offset,
        .residual_row_stride = post_params->residual_row_stride,
        .x_row_stride = post_params->x_row_stride,
        .y_row_stride = post_params->y_row_stride,
        .eps = post_params->eps,
    };
    metal_decode_replay_bind_attention(r);
}

static void metal_decode_replay_record_attention_query(
    struct metal_state *st,
    const struct geist_backend_attention_query_block *block) {

    struct metal_decode_replay_op *op =
        metal_decode_replay_append(st, METAL_DECODE_REPLAY_ATTENTION_QUERY);
    if (op == nullptr) { return; }
    struct metal_decode_replay_attention_query *r = &op->u.attention_query;
    r->block = *block;
    r->residual = *block->residual;
    r->attn_norm_weight = *block->attn_norm_weight;
    r->q_proj_weight = *block->q_proj_weight;
    ...
    r->post_attn_scratch = *block->post_attn_scratch;
    r->out = *block->out;
    metal_decode_replay_bind_attention_query(r);
}

static void metal_decode_replay_record_ffn(
    struct metal_state *st,
    const struct geist_backend_ffn_geglu_block *block,
    bool down_w_q6,
    const struct metal_rows_params *pre_norm_params,
    const struct metal_q4k_gate_up_params *gate_up_params,
    const struct metal_q4k_params *down_params,
    const struct metal_post_norm_params *post_params) {

    struct metal_decode_replay_op *op =
        metal_decode_replay_append(st, METAL_DECODE_REPLAY_FFN_GEGLU);
    if (op == nullptr) { return; }
    struct metal_decode_replay_ffn *r = &op->u.ffn;
    r->block = *block;
    r->residual = *block->residual;
    r->ffn_norm_weight = *block->ffn_norm_weight;
    r->gate_weight = *block->gate_weight;
    r->up_weight = *block->up_weight;
    r->down_weight = *block->down_weight;
    r->post_ffw_norm_weight = *block->post_ffw_norm_weight;
    r->pre_ff_scratch = *block->pre_ff_scratch;
    r->gate_scratch = *block->gate_scratch;
    r->up_scratch = *block->up_scratch;
    r->ffn_out_scratch = *block->ffn_out_scratch;
    r->post_ff_scratch = *block->post_ff_scratch;
    r->out = *block->out;
    r->down_w_q6 = down_w_q6;
    r->pre_norm_params = (struct metal_decode_replay_rows_params){
        .rows = pre_norm_params->rows,
        .cols = pre_norm_params->cols,
        .x_offset = pre_norm_params->x_offset,
        .w_offset = pre_norm_params->w_offset,
        .y_offset = pre_norm_params->y_offset,
        .x_row_stride = pre_norm_params->x_row_stride,
        .y_row_stride = pre_norm_params->y_row_stride,
        .eps = pre_norm_params->eps,
    };
    r->gate_up_params = (struct metal_decode_replay_q4k_gate_up_params){
        .n_in = gate_up_params->n_in,
        .n_out = gate_up_params->n_out,
        .rows = gate_up_params->rows,
        .blocks_per_row = gate_up_params->blocks_per_row,
        .x_offset = gate_up_params->x_offset,
        .gate_w_byte_offset = gate_up_params->gate_w_byte_offset,
        .up_w_byte_offset = gate_up_params->up_w_byte_offset,
        .gate_y_offset = gate_up_params->gate_y_offset,
        .up_y_offset = gate_up_params->up_y_offset,
        .x_row_stride = gate_up_params->x_row_stride,
        .y_row_stride = gate_up_params->y_row_stride,
    };
    r->down_params = (struct metal_decode_replay_q4k_params){
        .n_in = down_params->n_in,
        .n_out = down_params->n_out,
        .rows = down_params->rows,
        .blocks_per_row = down_params->blocks_per_row,
        .x_offset = down_params->x_offset,
        .w_byte_offset = down_params->w_byte_offset,
        .y_offset = down_params->y_offset,
        .x_row_stride = down_params->x_row_stride,
        .y_row_stride = down_params->y_row_stride,
    };
    r->post_params = (struct metal_decode_replay_post_norm_params){
        .rows = post_params->rows,
        .cols = post_params->cols,
    ...
    metal_decode_replay_bind_ffn(r);
}

static void metal_decode_replay_record_ple(
    struct metal_state *st,
    const struct geist_backend_ple_block *block,
    bool gate_w_q4,
    bool proj_w_q4,
    const struct metal_q4k_params *gate_q4_params,
    const struct metal_q4k_params *proj_q4_params,
    const struct metal_f32_params *gate_f32_params,
    const struct metal_f32_params *proj_f32_params,
    const struct metal_binary_rows_params *act_params,
    const struct metal_post_norm_params *post_params) {

    struct metal_decode_replay_op *op =
        metal_decode_replay_append(st, METAL_DECODE_REPLAY_PLE);
    if (op == nullptr) { return; }
    struct metal_decode_replay_ple *r = &op->u.ple;
    r->block = *block;
    r->hidden = *block->hidden;
    r->per_layer_input = *block->per_layer_input;
    r->per_layer_gate_weight = *block->per_layer_gate_weight;
    r->per_layer_proj_weight = *block->per_layer_proj_weight;
    r->post_per_layer_norm_weight = *block->post_per_layer_norm_weight;
    r->gate_scratch = *block->gate_scratch;
    r->proj_scratch = *block->proj_scratch;
    r->out = *block->out;
    r->gate_w_q4 = gate_w_q4;
    r->proj_w_q4 = proj_w_q4;
    r->gate_q4_params = (struct metal_decode_replay_q4k_params){
        .n_in = gate_q4_params->n_in,
        .n_out = gate_q4_params->n_out,
        .rows = gate_q4_params->rows,
        .blocks_per_row = gate_q4_params->blocks_per_row,
        .x_offset = gate_q4_params->x_offset,
        .w_byte_offset = gate_q4_params->w_byte_offset,
        .y_offset = gate_q4_params->y_offset,
        .x_row_stride = gate_q4_params->x_row_stride,
        .y_row_stride = gate_q4_params->y_row_stride,
    };
    r->proj_q4_params = (struct metal_decode_replay_q4k_params){
        .n_in = proj_q4_params->n_in,
        .n_out = proj_q4_params->n_out,
        .rows = proj_q4_params->rows,
        .blocks_per_row = proj_q4_params->blocks_per_row,
        .x_offset = proj_q4_params->x_offset,
        .w_byte_offset = proj_q4_params->w_byte_offset,
        .y_offset = proj_q4_params->y_offset,
        .x_row_stride = proj_q4_params->x_row_stride,
        .y_row_stride = proj_q4_params->y_row_stride,
    };
    r->gate_f32_params = (struct metal_decode_replay_f32_params){
        .n_in = gate_f32_params->n_in,
        .n_out = gate_f32_params->n_out,
        .rows = gate_f32_params->rows,
        .x_offset = gate_f32_params->x_offset,
        .w_offset = gate_f32_params->w_offset,
        .y_offset = gate_f32_params->y_offset,
        .x_row_stride = gate_f32_params->x_row_stride,
        .y_row_stride = gate_f32_params->y_row_stride,
    };
    r->proj_f32_params = (struct metal_decode_replay_f32_params){
        .n_in = proj_f32_params->n_in,
        .n_out = proj_f32_params->n_out,
        .rows = proj_f32_params->rows,
        .x_offset = proj_f32_params->x_offset,
        .w_offset = proj_f32_params->w_offset,
        .y_offset = proj_f32_params->y_offset,
        .x_row_stride = proj_f32_params->x_row_stride,
        .y_row_stride = proj_f32_params->y_row_stride,
    };
    r->act_params = (struct metal_decode_replay_binary_rows_params){
        .rows = act_params->rows,
        .cols = act_params->cols,
        .a_offset = act_params->a_offset,
        .b_offset = act_params->b_offset,
        .y_offset = act_params->y_offset,
        .a_row_stride = act_params->a_row_stride,
        .b_row_stride = act_params->b_row_stride,
        .y_row_stride = act_params->y_row_stride,
    };
    r->post_params = (struct metal_decode_replay_post_norm_params){
        .rows = post_params->rows,
        .cols = post_params->cols,
    ...
    metal_decode_replay_bind_ple(r);
}

static void metal_decode_replay_record_greedy_head(
    struct metal_state *st,
    ...
    bool lm_head_q6,
    const struct metal_rows_params *norm_params,
    const struct metal_f32_params *f32_params,
    const struct metal_q4k_params *q_params,
    const struct metal_argmax_params *argmax_params) {

    struct metal_decode_replay_op *op =
        metal_decode_replay_append(st, METAL_DECODE_REPLAY_GREEDY_HEAD);
    if (op == nullptr) { return; }
    struct metal_decode_replay_greedy_head *r = &op->u.greedy_head;
    r->head = *head;
    r->hidden = *head->hidden;
    r->norm_weight = *head->norm_weight;
    r->lm_head_weight = *head->lm_head_weight;
    r->normed_scratch = *head->normed_scratch;
    r->logits = *head->logits;
    r->lm_head_f32 = lm_head_f32;
    r->lm_head_q4 = lm_head_q4;
    r->lm_head_q6 = lm_head_q6;
    r->norm_params = (struct metal_decode_replay_rows_params){
        .rows = norm_params->rows,
        .cols = norm_params->cols,
        .x_offset = norm_params->x_offset,
    ...
    r->f32_params = (struct metal_decode_replay_f32_params){
        .n_in = f32_params->n_in,
        .n_out = f32_params->n_out,
        .rows = f32_params->rows,
        .x_offset = f32_params->x_offset,
        .w_offset = f32_params->w_offset,
        .y_offset = f32_params->y_offset,
        .x_row_stride = f32_params->x_row_stride,
        .y_row_stride = f32_params->y_row_stride,
    };
    r->q_params = (struct metal_decode_replay_q4k_params){
        .n_in = q_params->n_in,
        .n_out = q_params->n_out,
    ...
    r->argmax_params = (struct metal_decode_replay_argmax_params){
        .n = argmax_params->n,
        .x_offset = argmax_params->x_offset,
    };
    metal_decode_replay_bind_greedy_head(r);
}
...
[[nodiscard]] static enum geist_status metal_buffer_download_range(
    const struct geist_buffer *buf,
    size_t offset,
    size_t n_bytes,
    uint8_t dst[static n_bytes]) {

    if (buf == nullptr || offset > buf->bytes ||
        n_bytes > buf->bytes - offset ||
        (n_bytes > 0 && dst == nullptr)) {
        return GEIST_E_INVALID_ARG;
    }
    if (n_bytes == 0) {
    ...
    s = metal_submit_copy(st, buf->buffer, offset, staging->buffer, 0,
                          n_bytes);
    if (s == GEIST_OK) {
        memcpy(dst, staging->mapped, n_bytes);
    }
...
static uint64_t metal_hash_bytes(size_t n_bytes,
                                 const uint8_t data[static n_bytes]) {
...
static bool metal_pack_cache_path(const struct metal_state *st,
                                  const char *kind,
                                  size_t n_in,
                                  size_t n_out,
                                  size_t raw_bytes,
                                  ...
                                  uint64_t raw_hash,
                                  size_t path_n,
                                  char path[static path_n]) {
    if (st == nullptr || kind == nullptr || path == nullptr ||
        !st->pack_cache_enabled || st->pack_cache_dir[0] == '\0' ||
        path_n == 0) {
        return false;
    }
    const int n = snprintf(
        path, path_n,
        "%s/geist-metal-%s-v1-nin%zu-nout%zu-raw%zu-pack%zu-h%016llx.bin",
        st->pack_cache_dir, kind, n_in, n_out, raw_bytes, packed_bytes,
        (unsigned long long) raw_hash);
    return n > 0 && (size_t) n < path_n;
}

static bool metal_pack_cache_read(size_t packed_bytes,
                                  uint8_t packed[static packed_bytes],
                                  const char *path,
                                  size_t n_in,
                                  size_t n_out,
                                  size_t raw_bytes,
                                  uint64_t raw_hash) {
    if (packed == nullptr || path == nullptr || packed_bytes == 0) {
        return false;
    }
    ...
    static const uint8_t magic[8] = {'G', 'E', 'I', 'S',
                                     'T', 'P', 'K', '1'};
    struct metal_pack_cache_header header = {0};
    const size_t header_got = fread(&header, 1, sizeof(header), f);
    if (header_got != sizeof(header) ||
        memcmp(header.magic, magic, sizeof(magic)) != 0 ||
        header.version != 1u ||
        header.n_in != (uint64_t) n_in ||
        header.n_out != (uint64_t) n_out ||
        header.raw_bytes != (uint64_t) raw_bytes ||
        header.packed_bytes != (uint64_t) packed_bytes ||
        header.raw_hash != raw_hash) {
        (void) fclose(f);
        return false;
    }
    const size_t got = fread(packed, 1, packed_bytes, f);
    const int extra = fgetc(f);
    const bool ok = got == packed_bytes && extra == EOF && ferror(f) == 0 &&
                    metal_hash_bytes(packed_bytes, packed) ==
                        header.packed_hash;
    (void) fclose(f);
    return ok;
}

static void metal_pack_cache_write(size_t packed_bytes,
                                   const uint8_t packed[static packed_bytes],
                                   const char *path,
                                   size_t n_in,
                                   size_t n_out,
                                   size_t raw_bytes,
    ...
    static const uint8_t magic[8] = {'G', 'E', 'I', 'S',
                                     'T', 'P', 'K', '1'};
    struct metal_pack_cache_header header = {
        .magic = {magic[0], magic[1], magic[2], magic[3],
                  magic[4], magic[5], magic[6], magic[7]},
        .version = 1u,
        .n_in = (uint64_t) n_in,
        .n_out = (uint64_t) n_out,
        .raw_bytes = (uint64_t) raw_bytes,
        .packed_bytes = (uint64_t) packed_bytes,
        .raw_hash = raw_hash,
        .packed_hash = metal_hash_bytes(packed_bytes, packed),
    };
    const size_t header_wrote = fwrite(&header, 1, sizeof(header), f);
    const size_t wrote = fwrite(packed, 1, packed_bytes, f);
    if (header_wrote != sizeof(header) || wrote != packed_bytes ||
        fflush(f) != 0) {
        (void) fclose(f);
        (void) remove(path);
        return;
...
[[nodiscard]] static enum geist_status metal_q4k_nt4_cache_get(
    struct geist_backend *be,
    const struct geist_tensor *w,
    size_t n_in,
    size_t n_out,
    ...
    *packed_out = nullptr;
    ...
    struct metal_state *st = be->state;
    struct geist_buffer *cached =
        metal_q4k_nt4_cache_find(st, w, n_in, n_out);
    if (cached != nullptr) {
        *packed_out = cached;
        return GEIST_OK;
    }
    ...
    if (n_out * blocks_per_row * Q4_K_BLOCK_BYTES != raw_bytes ||
        packed_bytes == 0) {
        return GEIST_E_INVALID_ARG;
    }

    uint8_t *packed_host = heap_alloc_aligned(packed_bytes, OPTIMAL_ALIGNMENT);
    if (packed_host == nullptr) {
        safe_free((void **) &packed_host);
        geist_backend_set_error(be, GEIST_E_OOM,
                                "metal q4k nt4 cache: host allocation failed");
        return GEIST_E_OOM;
    }

    struct geist_buffer *packed = nullptr;
    struct metal_q4k_nt4_cache_entry *entry = nullptr;
    enum geist_status s = GEIST_OK;
    bool host_cache_hit = false;
    uint64_t raw_hash = 0;
    char cache_path[PATH_MAX] = {0};
    raw_hash = metal_hash_bytes(raw_bytes, raw);
    host_cache_hit = metal_pack_cache_path(
                         st, "q4k-nt4", n_in, n_out, raw_bytes,
                         packed_bytes, raw_hash, sizeof(cache_path),
                         cache_path) &&
                     metal_pack_cache_read(packed_bytes, packed_host,
                                           cache_path, n_in, n_out,
                                           raw_bytes, raw_hash);
    if (s == GEIST_OK && !host_cache_hit) {
        if (q4k_predecode_ntile4_pack(raw, n_in, n_out, packed_host) != 0) {
            geist_backend_set_error(be, GEIST_E_BACKEND,
                                    "metal q4k nt4 cache: pack failed");
            s = GEIST_E_BACKEND;
        } else if (cache_path[0] != '\0') {
            metal_pack_cache_write(packed_bytes, packed_host, cache_path,
                                   n_in, n_out, raw_bytes, raw_hash);
        }
    }
    if (s == GEIST_OK) {
        s = metal_new_buffer(be, packed_bytes, GEIST_BUFFER_WEIGHT,
                             GEIST_MEMORY_DEVICE, false, &packed);
    }
    if (s == GEIST_OK) {
        s = metal_buffer_upload(packed, packed_bytes, packed_host);
    }
    if (s == GEIST_OK) {
        entry = geist_backend_alloc(be, sizeof(*entry),
                                    alignof(struct metal_q4k_nt4_cache_entry));
        if (entry == nullptr) {
            geist_backend_set_error(
                be, GEIST_E_OOM,
                "metal q4k nt4 cache: entry allocation failed");
            s = GEIST_E_OOM;
        }
    }
    if (s != GEIST_OK) {
        if (packed != nullptr) {
            metal_buffer_destroy_internal(be, packed);
        }
        safe_free((void **) &packed_host);
        return s;
    }

    *entry = (struct metal_q4k_nt4_cache_entry) {
        .src = w->buffer,
        .src_offset = w->offset,
        .n_in = n_in,
        .n_out = n_out,
...
[[nodiscard]] static enum geist_status metal_q4k_nt4_cache_get_from_device(
    struct geist_backend *be,
    const struct geist_tensor *w,
    size_t n_in,
    size_t n_out,
    ...
    if (be == nullptr || be->state == nullptr || w == nullptr ||
        w->buffer == nullptr || packed_out == nullptr ||
        n_in == 0 || n_out == 0 ||
        (n_in % Q4_K_BLOCK_ELEMS) != 0) {
        return GEIST_E_INVALID_ARG;
    }
    ...
    if (n_out > SIZE_MAX / blocks_per_row ||
        n_out * blocks_per_row > SIZE_MAX / Q4_K_BLOCK_BYTES ||
        w->offset > w->buffer->bytes) {
        return GEIST_E_INVALID_ARG;
    }
    const size_t raw_bytes = n_out * blocks_per_row * Q4_K_BLOCK_BYTES;
    if (raw_bytes > w->buffer->bytes - w->offset) {
        return GEIST_E_INVALID_ARG;
    }
    uint8_t *raw = heap_alloc_aligned(raw_bytes, OPTIMAL_ALIGNMENT);
    if (raw == nullptr) {
        geist_backend_set_error(be, GEIST_E_OOM,
                                "metal q4k nt4 cache: host allocation failed");
        return GEIST_E_OOM;
    ...
    enum geist_status s =
        metal_buffer_download_range(w->buffer, w->offset, raw_bytes, raw);
    if (s == GEIST_OK) {
        s = metal_q4k_nt4_cache_get(be, w, n_in, n_out, raw_bytes, raw,
                                    packed_out);
    }
    safe_free((void **) &raw);
    return s;
...
static int metal_pack_raw_ntile4(const uint8_t *raw,
                                 size_t n_in,
                                 size_t n_out,
                                 size_t block_bytes,
                                 uint8_t *packed) {
    if (raw == nullptr || packed == nullptr || n_in == 0 ||
        n_out == 0 || block_bytes == 0 ||
        (n_in % METAL_Q6K_BLOCK_ELEMS) != 0) {
        return -1;
    }
    const size_t blocks_per_row = n_in / METAL_Q6K_BLOCK_ELEMS;
    const size_t n_tiles = (n_out + 3u) / 4u;
    memset(packed, 0, n_tiles * blocks_per_row * 4u * block_bytes);
    for (size_t nt = 0; nt < n_tiles; nt++) {
        for (size_t b = 0; b < blocks_per_row; b++) {
            for (size_t nr = 0; nr < 4u; nr++) {
                const size_t row = nt * 4u + nr;
                if (row >= n_out) {
                    continue;
                }
                const uint8_t *src =
                    raw + (row * blocks_per_row + b) * block_bytes;
                uint8_t *dst =
                    packed + ((nt * blocks_per_row + b) * 4u + nr) *
                                 block_bytes;
...
[[nodiscard]] static enum geist_status metal_q6k_nt4_cache_get(
    struct geist_backend *be,
    const struct geist_tensor *w,
    size_t n_in,
    size_t n_out,
    ...
    *packed_out = nullptr;
    ...
    struct metal_state *st = be->state;
    struct geist_buffer *cached =
        metal_q6k_nt4_cache_find(st, w, n_in, n_out);
    if (cached != nullptr) {
        *packed_out = cached;
        return GEIST_OK;
    }
    ...
    const size_t blocks_per_row = n_in / METAL_Q6K_BLOCK_ELEMS;
    if (n_out > SIZE_MAX / blocks_per_row ||
        n_out * blocks_per_row > SIZE_MAX / METAL_Q6K_BLOCK_BYTES) {
        return GEIST_E_INVALID_ARG;
    }
    const size_t n_tiles = (n_out + 3u) / 4u;
    if (n_out * blocks_per_row * METAL_Q6K_BLOCK_BYTES != raw_bytes ||
        n_tiles > SIZE_MAX / blocks_per_row ||
        n_tiles * blocks_per_row > SIZE_MAX / 4u ||
        n_tiles * blocks_per_row * 4u >
            SIZE_MAX / METAL_Q6K_BLOCK_BYTES) {
        return GEIST_E_INVALID_ARG;
    }
    const size_t packed_bytes =
        n_tiles * blocks_per_row * 4u * METAL_Q6K_BLOCK_BYTES;

    uint8_t *packed_host = heap_alloc_aligned(packed_bytes, OPTIMAL_ALIGNMENT);
    if (packed_host == nullptr) {
        safe_free((void **) &packed_host);
        geist_backend_set_error(be, GEIST_E_OOM,
                                "metal q6k nt4 cache: host allocation failed");
        return GEIST_E_OOM;
    }

    struct geist_buffer *packed = nullptr;
    struct metal_q6k_nt4_cache_entry *entry = nullptr;
    enum geist_status s = GEIST_OK;
    bool host_cache_hit = false;
    uint64_t raw_hash = 0;
    char cache_path[PATH_MAX] = {0};
    raw_hash = metal_hash_bytes(raw_bytes, raw);
    host_cache_hit = metal_pack_cache_path(
                         st, "q6k-nt4", n_in, n_out, raw_bytes,
                         packed_bytes, raw_hash, sizeof(cache_path),
                         cache_path) &&
                     metal_pack_cache_read(packed_bytes, packed_host,
                                           cache_path, n_in, n_out,
                                           raw_bytes, raw_hash);
    if (s == GEIST_OK && !host_cache_hit) {
        if (metal_pack_raw_ntile4(raw, n_in, n_out,
                                  METAL_Q6K_BLOCK_BYTES,
                                  packed_host) != 0) {
            geist_backend_set_error(be, GEIST_E_BACKEND,
                                    "metal q6k nt4 cache: pack failed");
            s = GEIST_E_BACKEND;
        } else if (cache_path[0] != '\0') {
            metal_pack_cache_write(packed_bytes, packed_host, cache_path,
                                   n_in, n_out, raw_bytes, raw_hash);
    ...
    if (s == GEIST_OK) {
        entry = geist_backend_alloc(be, sizeof(*entry),
                                    alignof(struct metal_q6k_nt4_cache_entry));
        if (entry == nullptr) {
            geist_backend_set_error(
                be, GEIST_E_OOM,
                "metal q6k nt4 cache: entry allocation failed");
            s = GEIST_E_OOM;
        }
    }
    if (s != GEIST_OK) {
    ...
    *entry = (struct metal_q6k_nt4_cache_entry) {
        .src = w->buffer,
        .src_offset = w->offset,
        .n_in = n_in,
        .n_out = n_out,
        ...
        .packed_bytes = packed_bytes,
        .packed = packed,
        .next = st->q6k_nt4_cache,
    };
    st->q6k_nt4_cache = entry;
    *packed_out = packed;
...
[[nodiscard]] static enum geist_status metal_q6k_nt4_cache_get_from_device(
    struct geist_backend *be,
    const struct geist_tensor *w,
    size_t n_in,
    size_t n_out,
    ...
    const size_t raw_bytes =
        n_out * blocks_per_row * METAL_Q6K_BLOCK_BYTES;
    if (raw_bytes > w->buffer->bytes - w->offset) {
        return GEIST_E_INVALID_ARG;
    }
    uint8_t *raw = heap_alloc_aligned(raw_bytes, OPTIMAL_ALIGNMENT);
    ...
    enum geist_status s =
        metal_buffer_download_range(w->buffer, w->offset, raw_bytes, raw);
    if (s == GEIST_OK) {
        s = metal_q6k_nt4_cache_get(be, w, n_in, n_out, raw_bytes, raw,
                                    packed_out);
    }
    safe_free((void **) &raw);
    return s;
...
static bool metal_tensor_is_f32_rows(const struct geist_tensor *t,
    ...
    return metal_tensor_is_f32_matrix(
...
[[nodiscard]] static enum geist_status metal_ensure_argmax_result_capacity(
    struct geist_backend *be,
    uint32_t capacity) {

    if (be == nullptr || be->state == nullptr || capacity == 0u) {
        return GEIST_E_INVALID_ARG;
    }
    ...
    if (st->argmax_result_buffer != nullptr &&
        st->argmax_result_mapped != nullptr &&
        st->argmax_result_capacity >= capacity) {
        return GEIST_OK;
    }
...
[[nodiscard]] static enum geist_status metal_ensure_q4k_gate_up_pipeline(
    struct geist_backend *be) {
    ...
    struct metal_state *st = be->state;
    if (st->q4k_gate_up_pipeline != nullptr &&
        st->q4k_gate_up_n4_pipeline != nullptr &&
        st->q4k_gate_up_nt4_pipeline != nullptr &&
        st->q4k_gate_up_nt8_pipeline != nullptr) {
        return GEIST_OK;
    ...
    void *source = metal_msg_send_id_cstr(
        st, ns_string, "stringWithUTF8String:", metal_q4k_gate_up_source);
    void *n4_source = metal_msg_send_id_cstr(
        st, ns_string, "stringWithUTF8String:",
        metal_q4k_gate_up_n4_source);
    void *nt4_source = metal_msg_send_id_cstr(
        st, ns_string, "stringWithUTF8String:",
        metal_q4k_gate_up_nt4_source);
    void *nt8_source = metal_msg_send_id_cstr(
        st, ns_string, "stringWithUTF8String:",
        metal_q4k_gate_up_nt8_source);
    if (source == nullptr || n4_source == nullptr ||
        nt4_source == nullptr || nt8_source == nullptr) {
        geist_backend_set_error(be, GEIST_E_BACKEND,
                                "metal: failed to create gate/up shader source");
        return GEIST_E_BACKEND;
    }
    void *err = nullptr;
    st->q4k_gate_up_library = metal_msg_send_id_id_id_err(
        st, st->device, "newLibraryWithSource:options:error:",
        source, nullptr, &err);
    if (st->q4k_gate_up_library == nullptr) {
        const char *msg = metal_nserror_message(st, err);
        geist_backend_set_error(be, GEIST_E_BACKEND,
                                "metal: Q4_K gate/up shader compile failed%s%s",
                                msg != nullptr ? ": " : "",
                                msg != nullptr ? msg : "");
        return GEIST_E_BACKEND;
    }
    err = nullptr;
    st->q4k_gate_up_nt4_library = metal_msg_send_id_id_id_err(
        st, st->device, "newLibraryWithSource:options:error:",
        nt4_source, nullptr, &err);
    if (st->q4k_gate_up_nt4_library == nullptr) {
        const char *msg = metal_nserror_message(st, err);
        geist_backend_set_error(
            be, GEIST_E_BACKEND,
            "metal: Q4_K gate/up nt4 shader compile failed%s%s",
            msg != nullptr ? ": " : "",
            msg != nullptr ? msg : "");
        return GEIST_E_BACKEND;
    }
    err = nullptr;
    st->q4k_gate_up_nt8_library = metal_msg_send_id_id_id_err(
        st, st->device, "newLibraryWithSource:options:error:",
        nt8_source, nullptr, &err);
    if (st->q4k_gate_up_nt8_library == nullptr) {
        const char *msg = metal_nserror_message(st, err);
        geist_backend_set_error(
            be, GEIST_E_BACKEND,
            "metal: Q4_K gate/up nt8 shader compile failed%s%s",
            msg != nullptr ? ": " : "",
            msg != nullptr ? msg : "");
        return GEIST_E_BACKEND;
    }
    err = nullptr;
    st->q4k_gate_up_n4_library = metal_msg_send_id_id_id_err(
        st, st->device, "newLibraryWithSource:options:error:",
        n4_source, nullptr, &err);
    ...
    enum geist_status s = metal_create_named_pipeline(
        be, st->q4k_gate_up_library, ns_string, "gate_up_q4k_m8",
        &st->q4k_gate_up_function, &st->q4k_gate_up_pipeline);
    if (s == GEIST_OK) {
        s = metal_create_named_pipeline(
            be, st->q4k_gate_up_n4_library, ns_string, "gate_up_q4k_n4",
            &st->q4k_gate_up_n4_function,
            &st->q4k_gate_up_n4_pipeline);
    }
    if (s == GEIST_OK) {
        s = metal_create_named_pipeline(
            be, st->q4k_gate_up_nt4_library, ns_string, "gate_up_q4k_nt4",
            &st->q4k_gate_up_nt4_function,
            &st->q4k_gate_up_nt4_pipeline);
    }
    if (s == GEIST_OK) {
        s = metal_create_named_pipeline(
            be, st->q4k_gate_up_nt8_library, ns_string, "gate_up_q4k_nt8",
            &st->q4k_gate_up_nt8_function,
            &st->q4k_gate_up_nt8_pipeline);
    }
    return s;
}

[[nodiscard]] static enum geist_status metal_ensure_q4k_qk_pipeline(
    struct geist_backend *be) {
    ...
    void *source = metal_msg_send_id_cstr(
        st, ns_string, "stringWithUTF8String:", metal_q4k_qk_source);
    if (source == nullptr) {
        geist_backend_set_error(be, GEIST_E_BACKEND,
                                "metal: failed to create Q/K shader source");
        return GEIST_E_BACKEND;
    }
    void *err = nullptr;
    st->q4k_qk_library = metal_msg_send_id_id_id_err(
        st, st->device, "newLibraryWithSource:options:error:",
        source, nullptr, &err);
    if (st->q4k_qk_library == nullptr) {
        const char *msg = metal_nserror_message(st, err);
        geist_backend_set_error(be, GEIST_E_BACKEND,
                                "metal: Q/K shader compile failed%s%s",
                                msg != nullptr ? ": " : "",
                                msg != nullptr ? msg : "");
        return GEIST_E_BACKEND;
    }
    enum geist_status s = metal_create_named_pipeline(
        be, st->q4k_qk_library, ns_string, "qk_q4k",
        &st->q4k_qk_function, &st->q4k_qk_pipeline);
    if (s == GEIST_OK) {
        s = metal_create_named_pipeline(
            be, st->q4k_qk_library, ns_string, "qk_q4k_nt4",
            &st->q4k_qk_nt4_function, &st->q4k_qk_nt4_pipeline);
    }
    return s;
}
...
static bool metal_q4k_gate_up_w4a8_ready(
    const struct metal_state *st,
    const struct metal_q4k_gate_up_params *params,
    const struct geist_buffer *gate_packed,
    const struct geist_buffer *up_packed) {

    if (st == nullptr || params == nullptr || gate_packed == nullptr ||
        up_packed == nullptr || !st->use_q4k_w4a8 || params->rows != 1u ||
        st->q4k_quant_x_pipeline == nullptr ||
        st->q4k_gate_up_w4a8_pipeline == nullptr ||
        st->q4k_w4a8_xq_buffer == nullptr ||
        st->q4k_w4a8_scale_buffer == nullptr) {
...
static void metal_encode_q4k_gate_up_w4a8(
    struct metal_state *st,
    void *enc,
    const struct geist_tensor *x,
    const struct geist_buffer *gate_packed,
    const struct geist_buffer *up_packed,
    const struct geist_tensor *gate_y,
    const struct metal_q4k_gate_up_params *params) {
    ...
    struct metal_q4k_gate_up_params gate_up_params = *params;
    gate_up_params.gate_w_byte_offset = 0u;
    gate_up_params.up_w_byte_offset = 0u;
    ...
    (void) metal_msg_send_id_id(st, enc, "setComputePipelineState:",
                                st->q4k_gate_up_w4a8_pipeline);
    metal_msg_send_set_buffer(st, enc, st->q4k_w4a8_xq_buffer->buffer, 0, 0);
    metal_msg_send_set_buffer(st, enc, gate_packed->buffer, 0, 1);
    metal_msg_send_set_buffer(st, enc, up_packed->buffer, 0, 2);
    metal_msg_send_set_buffer(st, enc, gate_y->buffer->buffer, 0, 3);
    metal_msg_send_set_buffer(st, enc, st->q4k_w4a8_scale_buffer->buffer,
                              0, 4);
    metal_msg_send_set_bytes(st, enc, &gate_up_params, sizeof(gate_up_params),
                             5);
    const struct metal_size groups = {
        .width = (params->n_out + 3u) / 4u,
        .height = params->rows,
    ...
    metal_profile_add_dispatch(st, METAL_PROFILE_DISPATCH_Q4K_GATE_UP_W4A8,
                               groups);
    metal_msg_send_dispatch(st, enc, groups, threads);
}
...
static bool metal_try_encode_q4k_ple_gate_nt8(
    struct metal_state *st,
    void *enc,
    const struct geist_tensor *x,
    const struct geist_tensor *w,
    const struct geist_tensor *ple,
    const struct geist_tensor *y,
    const struct metal_q4k_params *q4k_params,
    const struct metal_binary_rows_params *act_params) {

    if (st == nullptr || enc == nullptr || x == nullptr || w == nullptr ||
        ple == nullptr || y == nullptr || q4k_params == nullptr ||
        act_params == nullptr || st->q4k_ple_gate_nt8_pipeline == nullptr ||
        !st->use_q4k_nt4 || !st->use_q4k_nt8 ||
        q4k_params->n_out < 8u ||
        q4k_params->rows != act_params->rows ||
        q4k_params->n_out != act_params->cols ||
        q4k_params->y_offset != act_params->a_offset ||
        q4k_params->y_offset != act_params->y_offset ||
        q4k_params->y_row_stride != act_params->a_row_stride ||
        q4k_params->y_row_stride != act_params->y_row_stride) {
        return false;
    }

    struct geist_buffer *packed = metal_q4k_nt4_cache_find_tensor(st, w);
    if (packed == nullptr) {
        return false;
    }

    const struct metal_q4k_ple_gate_params params = {
        .n_in = q4k_params->n_in,
        .n_out = q4k_params->n_out,
        .rows = q4k_params->rows,
        .blocks_per_row = q4k_params->blocks_per_row,
        .x_offset = q4k_params->x_offset,
        .w_byte_offset = 0u,
        .ple_offset = act_params->b_offset,
        .y_offset = act_params->y_offset,
        .x_row_stride = q4k_params->x_row_stride,
        .ple_row_stride = act_params->b_row_stride,
        .y_row_stride = act_params->y_row_stride,
    };
    ...
    metal_profile_add_dispatch(st, METAL_PROFILE_DISPATCH_Q4K_PLE_GATE_NT8,
                               groups);
    metal_msg_send_dispatch(st, enc, groups, threads);
    return true;
}
...
static void metal_encode_q4k_qk(
    struct metal_state *st,
    void *enc,
    const struct geist_tensor *x,
    const struct geist_tensor *q_w,
    const struct geist_tensor *k_w,
    const struct geist_tensor *q_y,
    const struct geist_tensor *k_y,
    const struct metal_q4k_qk_params *params) {

    (void) metal_msg_send_id_id(st, enc, "setComputePipelineState:",
                                st->q4k_qk_pipeline);
    metal_msg_send_set_buffer(st, enc, x->buffer->buffer, 0, 0);
    metal_msg_send_set_buffer(st, enc, q_w->buffer->buffer, 0, 1);
    metal_msg_send_set_buffer(st, enc, k_w->buffer->buffer, 0, 2);
    metal_msg_send_set_buffer(st, enc, q_y->buffer->buffer, 0, 3);
    metal_msg_send_set_buffer(st, enc, k_y->buffer->buffer, 0, 4);
    metal_msg_send_set_bytes(st, enc, params, sizeof(*params), 5);
    const uint32_t out =
        params->q_out > params->k_out ? params->q_out : params->k_out;
    const struct metal_size groups = {
        .width = out,
        .height = params->rows,
        .depth = 1,
    ...
    metal_profile_add_dispatch(st, METAL_PROFILE_DISPATCH_Q4K_QK_BASE,
                               groups);
    metal_msg_send_dispatch(st, enc, groups, threads);
}

static void metal_encode_q4k_qk_nt4(
    struct metal_state *st,
    void *enc,
    const struct geist_tensor *x,
    const struct geist_buffer *q_packed,
    const struct geist_buffer *k_packed,
    const struct geist_tensor *q_y,
    const struct geist_tensor *k_y,
    const struct metal_q4k_qk_params *params) {

    struct metal_q4k_qk_params packed_params = *params;
    packed_params.q_w_byte_offset = 0u;
    packed_params.k_w_byte_offset = 0u;
    (void) metal_msg_send_id_id(st, enc, "setComputePipelineState:",
                                st->q4k_qk_nt4_pipeline);
    metal_msg_send_set_buffer(st, enc, x->buffer->buffer, 0, 0);
    metal_msg_send_set_buffer(st, enc, q_packed->buffer, 0, 1);
    metal_msg_send_set_buffer(st, enc, k_packed->buffer, 0, 2);
    metal_msg_send_set_buffer(st, enc, q_y->buffer->buffer, 0, 3);
    metal_msg_send_set_buffer(st, enc, k_y->buffer->buffer, 0, 4);
    metal_msg_send_set_bytes(st, enc, &packed_params, sizeof(packed_params),
                             5);
    const uint32_t q_groups = (params->q_out + 3u) / 4u;
    const uint32_t k_groups = (params->k_out + 3u) / 4u;
    const struct metal_size groups = {
        .width = q_groups > k_groups ? q_groups : k_groups,
        .height = params->rows,
        .depth = 1,
    };
    const struct metal_size threads = {
    ...
    metal_profile_add_dispatch(st, METAL_PROFILE_DISPATCH_Q4K_QK_NT4,
                               groups);
    metal_msg_send_dispatch(st, enc, groups, threads);
}

static void metal_encode_q4k_gate_up(
    struct metal_state *st,
    void *enc,
    const struct geist_tensor *x,
    ...
    const struct geist_tensor *gate_y,
    const struct geist_tensor *up_y,
    const struct metal_q4k_gate_up_params *params) {

    const bool n_tile4 = st->use_q4k_n4 && params->rows == 1u;
    struct geist_buffer *gate_packed =
        st->use_q4k_nt4 && !st->use_q4k_gate_up_raw && n_tile4
            ? metal_q4k_nt4_cache_find_tensor(st, gate_w)
            : nullptr;
    struct geist_buffer *up_packed =
        st->use_q4k_nt4 && !st->use_q4k_gate_up_raw && n_tile4
            ? metal_q4k_nt4_cache_find_tensor(st, up_w)
            : nullptr;
    const bool packed =
        gate_packed != nullptr && up_packed != nullptr;
    struct metal_q4k_gate_up_params packed_params = *params;
    if (packed) {
        packed_params.gate_w_byte_offset = 0u;
        packed_params.up_w_byte_offset = 0u;
    }
    if (metal_q4k_gate_up_w4a8_ready(st, params, gate_packed, up_packed)) {
        (void) up_y;
        metal_encode_q4k_gate_up_w4a8(st, enc, x, gate_packed, up_packed,
                                      gate_y, params);
        return;
    }
    const bool n_tile8 =
        packed && st->use_q4k_nt8 && params->n_out >= 8u;
    (void) metal_msg_send_id_id(st, enc, "setComputePipelineState:",
                                n_tile8 ? st->q4k_gate_up_nt8_pipeline
                                : packed ? st->q4k_gate_up_nt4_pipeline
                                : n_tile4 ? st->q4k_gate_up_n4_pipeline
                                          : st->q4k_gate_up_pipeline);
    metal_msg_send_set_buffer(st, enc, x->buffer->buffer, 0, 0);
    metal_msg_send_set_buffer(st, enc,
                              packed ? gate_packed->buffer
                                     : gate_w->buffer->buffer,
                              0, 1);
    metal_msg_send_set_buffer(st, enc,
                              packed ? up_packed->buffer
                                     : up_w->buffer->buffer,
                              0, 2);
    metal_msg_send_set_buffer(st, enc, gate_y->buffer->buffer, 0, 3);
    metal_msg_send_set_buffer(st, enc, up_y->buffer->buffer, 0, 4);
    metal_msg_send_set_bytes(st, enc,
                             packed ? &packed_params : params,
                             sizeof(*params), 5);
    const struct metal_size groups = {
        .width = n_tile8 ? (params->n_out + 7u) / 8u
                 : n_tile4 ? (params->n_out + 3u) / 4u
    ...
    const enum metal_profile_stage profile_stage =
        n_tile8 ? METAL_PROFILE_DISPATCH_Q4K_GATE_UP_NT8
        : packed ? METAL_PROFILE_DISPATCH_Q4K_GATE_UP_NT4
        : n_tile4 ? METAL_PROFILE_DISPATCH_Q4K_GATE_UP_N4
                  : METAL_PROFILE_DISPATCH_Q4K_GATE_UP_BASE;
    metal_profile_add_dispatch(st, profile_stage, groups);
    metal_msg_send_dispatch(st, enc, groups, threads);
}
...
static void metal_encode_rmsnorm_add_rows(
    ...
    (void) metal_msg_send_id_id(st, enc, "setComputePipelineState:",
                                st->use_rmsnorm_simd
                                    ? st->rmsnorm_add_rows_simd_pipeline
                                    : st->rmsnorm_add_rows_pipeline);
    metal_msg_send_set_buffer(st, enc, residual->buffer->buffer, 0, 0);
...
static void metal_encode_q_norm_rope_rows(
    struct metal_state *st,
    void *enc,
    struct geist_tensor *x,
    const struct geist_tensor *w,
    ...
    const struct metal_norm_rope_params *params) {
    ...
    metal_profile_add_dispatch(st, METAL_PROFILE_DISPATCH_Q_NORM_ROPE,
                               groups);
    metal_msg_send_dispatch(st, enc, groups, threads);
}

static void metal_encode_k_norm_rope_append_rows(
    struct metal_state *st,
    void *enc,
    struct geist_tensor *x,
    const struct geist_tensor *w,
    ...
    const struct metal_k_norm_rope_append_params *params) {
    ...
    metal_msg_send_set_buffer(st, enc, cos->buffer->buffer, 0, 2);
    metal_msg_send_set_buffer(st, enc, sin->buffer->buffer, 0, 3);
    metal_msg_send_set_buffer(st, enc, k_cache->buffer->buffer, 0, 4);
    metal_msg_send_set_bytes(st, enc, params, sizeof(*params), 5);
    const struct metal_size groups = {
        .width = params->rows,
        .height = params->heads,
    ...
    metal_profile_add_dispatch(
        st, METAL_PROFILE_DISPATCH_K_NORM_ROPE_APPEND, groups);
    metal_msg_send_dispatch(st, enc, groups, threads);
}

static void metal_encode_v_norm_append_rows(
    struct metal_state *st,
    void *enc,
    struct geist_tensor *x,
    const struct geist_tensor *w,
    const struct geist_tensor *v_cache,
    const struct metal_v_norm_append_params *params) {

    void *pipeline = v_cache->dtype == GEIST_DTYPE_F16
                         ? st->v_norm_append_rows_f16_pipeline
                         : st->v_norm_append_rows_pipeline;
    (void) metal_msg_send_id_id(st, enc, "setComputePipelineState:",
                                pipeline);
    ...
    metal_profile_add_dispatch(st, METAL_PROFILE_DISPATCH_V_NORM_APPEND,
                               groups);
    metal_msg_send_dispatch(st, enc, groups, threads);
}

static bool metal_can_fuse_kv_norm_append(
    const struct metal_state *st,
    const struct geist_tensor *k_cache,
    const struct geist_tensor *v_cache,
    const struct metal_k_norm_rope_append_params *k_params,
    const struct metal_v_norm_append_params *v_params) {

    if (st == nullptr || k_cache == nullptr || v_cache == nullptr ||
        k_params == nullptr || v_params == nullptr ||
        !st->use_fused_kv_norm_append ||
        k_cache->dtype != v_cache->dtype ||
        k_params->rows != v_params->rows ||
        k_params->heads != v_params->heads ||
        k_params->head_dim != v_params->head_dim ||
        k_params->q_position != v_params->q_position ||
        k_params->rows == 0u || k_params->heads == 0u ||
        k_params->head_dim == 0u) {
        return false;
    }
    if (k_cache->dtype == GEIST_DTYPE_F16) {
        return st->kv_norm_append_rows_f16_pipeline != nullptr;
    }
    return st->kv_norm_append_rows_pipeline != nullptr;
}

static void metal_encode_kv_norm_append_rows(
    struct metal_state *st,
    void *enc,
    struct geist_tensor *k,
    struct geist_tensor *v,
    const struct geist_tensor *k_weight,
    const struct geist_tensor *v_weight,
    const struct geist_tensor *cos,
    const struct geist_tensor *sin,
    const struct geist_tensor *k_cache,
    const struct geist_tensor *v_cache,
    ...
    metal_msg_send_set_buffer(st, enc, k_weight->buffer->buffer, 0, 2);
    metal_msg_send_set_buffer(st, enc, v_weight->buffer->buffer, 0, 3);
    ...
    metal_msg_send_set_bytes(st, enc, k_params, sizeof(*k_params), 8);
    metal_msg_send_set_bytes(st, enc, v_params, sizeof(*v_params), 9);
    const struct metal_size groups = {
        .width = k_params->rows,
        .height = k_params->heads,
        .depth = 1,
    };
    const struct metal_size threads = {
        .width = METAL_ELEM_THREADS,
    ...
    metal_profile_add_dispatch(st, METAL_PROFILE_DISPATCH_KV_NORM_APPEND,
                               groups);
    metal_msg_send_dispatch(st, enc, groups, threads);
}
...
static void metal_encode_kv_append_rows(
    struct metal_state *st,
    void *enc,
    ...
    const struct geist_tensor *k_cache,
    const struct geist_tensor *v_cache,
    const struct metal_kv_append_params *params) {
    ...
    metal_msg_send_set_bytes(st, enc, params, sizeof(*params), 4);
    const struct metal_size groups = {
        .width = (params->elems + METAL_ELEM_THREADS - 1u) /
                 METAL_ELEM_THREADS,
        .height = 1,
        .depth = 1,
    };
...
static bool metal_can_fuse_qnorm_attention(
    const struct metal_state *st,
    const struct geist_tensor *k_cache,
    const struct metal_norm_rope_params *norm_params,
    const struct metal_attention_params *attention_params) {

    if (st == nullptr || k_cache == nullptr || norm_params == nullptr ||
        attention_params == nullptr || norm_params->head_dim == 0u ||
        norm_params->head_dim > METAL_QNORM_ATTENTION_MAX_HEAD_DIM ||
        norm_params->head_dim != attention_params->head_dim ||
        norm_params->rows != attention_params->rows ||
        norm_params->heads != attention_params->q_heads ||
        norm_params->x_offset != attention_params->q_offset) {
        return false;
    }
    if (norm_params->rows > 1u &&
        norm_params->x_row_stride !=
            attention_params->q_heads * attention_params->head_dim) {
        return false;
    }
    if (k_cache->dtype == GEIST_DTYPE_F16) {
        return st->attention_qnorm_rows_f16_pipeline != nullptr;
    }
    return st->attention_qnorm_rows_pipeline != nullptr;
}

static void metal_encode_attention_qnorm_rows(
    struct metal_state *st,
    void *enc,
    const struct geist_tensor *q,
    ...
    /* rows==1 (decode, head_dim<=256, f16 KV): split-KV flash decode — the
     * attention window is spread over up to 8 threadgroups per head (pass 1
     * writes per-split m/l/acc partials) plus a tiny combine dispatch. A
     * single tg per head leaves the GPU nearly idle at decode. */
    if (attention_params->rows == 1u && attention_params->head_dim <= 256u &&
        k_cache->dtype == GEIST_DTYPE_F16 &&
        st->attention_qnorm_dec_f16_pipeline != nullptr &&
        st->attention_dec_combine_pipeline != nullptr) {
        const uint32_t hd = attention_params->head_dim;
        const uint32_t qh = attention_params->q_heads;
        uint32_t window = attention_params->q_position + 1u;
        if (attention_params->kv_len < window) {
            window = attention_params->kv_len;
        }
        if (attention_params->sliding_window > 0u &&
            attention_params->sliding_window < window) {
            window = attention_params->sliding_window;
        }
        ...
        const size_t partial_bytes =
            (size_t) qh * 16u * ((size_t) hd + 2u) * sizeof(float);
        if (st->attn_dec_partials_buffer != nullptr &&
            st->attn_dec_partials_capacity < partial_bytes) {
            metal_buffer_destroy_internal(st->backend,
                                          st->attn_dec_partials_buffer);
            st->attn_dec_partials_buffer = nullptr;
            st->attn_dec_partials_capacity = 0;
        }
        if (st->attn_dec_partials_buffer == nullptr &&
            metal_new_buffer(st->backend, partial_bytes, GEIST_BUFFER_SCRATCH,
                             GEIST_MEMORY_DEVICE, false,
        ...
        if (st->attn_dec_partials_buffer != nullptr) {
            (void) metal_msg_send_id_id(st, enc, "setComputePipelineState:",
                                        st->attention_qnorm_dec_f16_pipeline);
            metal_msg_send_set_buffer(st, enc, q->buffer->buffer, 0, 0);
            metal_msg_send_set_buffer(st, enc, q_norm_weight->buffer->buffer,
                                      0, 1);
            metal_msg_send_set_buffer(st, enc, cos->buffer->buffer, 0, 2);
            metal_msg_send_set_buffer(st, enc, sin->buffer->buffer, 0, 3);
            metal_msg_send_set_buffer(st, enc, k_cache->buffer->buffer, 0, 4);
            metal_msg_send_set_buffer(st, enc, v_cache->buffer->buffer, 0, 5);
            metal_msg_send_set_buffer(
                st, enc, st->attn_dec_partials_buffer->buffer, 0, 6);
            metal_msg_send_set_bytes(st, enc, norm_params,
                                     sizeof(*norm_params), 7);
            metal_msg_send_set_bytes(st, enc, attention_params,
                                     sizeof(*attention_params), 8);
            metal_msg_send_set_bytes(st, enc, &nsplit, sizeof(nsplit), 9);
            const struct metal_size groups1 = {
                .width = nsplit, .height = qh, .depth = 1,
            };
            const struct metal_size threads1 = {
                .width = METAL_ELEM_THREADS, .height = 1, .depth = 1,
            };
            metal_profile_add_dispatch(
                st, METAL_PROFILE_DISPATCH_ATTENTION_QNORM_ROWS, groups1);
            metal_msg_send_dispatch(st, enc, groups1, threads1);

            const uint32_t cb[4] = {qh, hd, nsplit,
                                    attention_params->y_offset};
            (void) metal_msg_send_id_id(st, enc, "setComputePipelineState:",
                                        st->attention_dec_combine_pipeline);
            metal_msg_send_set_buffer(
                st, enc, st->attn_dec_partials_buffer->buffer, 0, 0);
            metal_msg_send_set_buffer(st, enc, y->buffer->buffer, 0, 1);
            metal_msg_send_set_bytes(st, enc, cb, sizeof(cb), 2);
            const struct metal_size groups2 = {
                .width = 1, .height = qh, .depth = 1,
            };
            metal_profile_add_dispatch(
                st, METAL_PROFILE_DISPATCH_ATTENTION_QNORM_ROWS, groups2);
            metal_msg_send_dispatch(st, enc, groups2, threads1);
            return;
        }
    }

    /* Prefill (rows>1): simdgroup flash attention when the tile geometry
     * fits; GEIST_METAL_FLASH_SG=0 falls back to the scalar kernel. */
    if (attention_params->rows > 1u && (attention_params->rows % 8u) == 0u &&
        attention_params->head_dim <= 256u &&
        (attention_params->head_dim % 32u) == 0u &&
        attention_params->kv_len >= 32u &&
        k_cache->dtype == GEIST_DTYPE_F16 &&
        st->attention_qnorm_flash_sg_f16_pipeline != nullptr &&
        !metal_env_disabled("GEIST_METAL_FLASH_SG")) {
        (void) metal_msg_send_id_id(
            st, enc, "setComputePipelineState:",
            st->attention_qnorm_flash_sg_f16_pipeline);
        metal_msg_send_set_buffer(st, enc, q->buffer->buffer, 0, 0);
        metal_msg_send_set_buffer(st, enc, q_norm_weight->buffer->buffer, 0,
                                  1);
        metal_msg_send_set_buffer(st, enc, cos->buffer->buffer, 0, 2);
        metal_msg_send_set_buffer(st, enc, sin->buffer->buffer, 0, 3);
        metal_msg_send_set_buffer(st, enc, k_cache->buffer->buffer, 0, 4);
        metal_msg_send_set_buffer(st, enc, v_cache->buffer->buffer, 0, 5);
        metal_msg_send_set_buffer(st, enc, y->buffer->buffer, 0, 6);
        metal_msg_send_set_bytes(st, enc, norm_params, sizeof(*norm_params),
                                 7);
        metal_msg_send_set_bytes(st, enc, attention_params,
                                 sizeof(*attention_params), 8);
        const struct metal_size fgroups = {
            .width = attention_params->rows / 8u,
            .height = attention_params->q_heads,
            .depth = 1,
        };
        const struct metal_size fthreads = {
            .width = 128u, .height = 1, .depth = 1,
        };
        metal_profile_add_dispatch(
            st, METAL_PROFILE_DISPATCH_ATTENTION_QNORM_ROWS, fgroups);
    ...
    (void) metal_msg_send_id_id(st, enc, "setComputePipelineState:",
                                pipeline);
    metal_msg_send_set_buffer(st, enc, q->buffer->buffer, 0, 0);
    metal_msg_send_set_buffer(st, enc, q_norm_weight->buffer->buffer, 0, 1);
    metal_msg_send_set_buffer(st, enc, cos->buffer->buffer, 0, 2);
    metal_msg_send_set_buffer(st, enc, sin->buffer->buffer, 0, 3);
    metal_msg_send_set_buffer(st, enc, k_cache->buffer->buffer, 0, 4);
    metal_msg_send_set_buffer(st, enc, v_cache->buffer->buffer, 0, 5);
    metal_msg_send_set_buffer(st, enc, y->buffer->buffer, 0, 6);
    metal_msg_send_set_bytes(st, enc, norm_params, sizeof(*norm_params), 7);
    metal_msg_send_set_bytes(st, enc, attention_params,
                             sizeof(*attention_params), 8);
    const struct metal_size groups = {
        .width = attention_params->rows,
        .height = attention_params->q_heads,
        .depth = 1,
    };
    const struct metal_size threads = {
    ...
    metal_profile_add_dispatch(st, METAL_PROFILE_DISPATCH_ATTENTION_QNORM_ROWS,
                               groups);
    metal_msg_send_dispatch(st, enc, groups, threads);
}
...
static void metal_encode_f32_ple_gate(
    struct metal_state *st,
    void *enc,
    const struct geist_tensor *x,
    const struct geist_tensor *w,
    ...
    const struct metal_f32_ple_gate_params params = {
        .n_in = f32_params->n_in,
        .n_out = f32_params->n_out,
        .rows = f32_params->rows,
        .x_offset = f32_params->x_offset,
    ...
    metal_msg_send_set_bytes(st, enc, &params, sizeof(params), 4);
    const struct metal_size groups = {
        .width = f32_params->n_out,
        .height = f32_params->rows,
        .depth = 1,
    };
    const struct metal_size threads = {
        .width = METAL_ELEM_THREADS,
...
static bool metal_can_encode_f32_ple_proj_norm(
    const struct metal_state *st,
    const struct geist_tensor *residual,
    const struct geist_tensor *out,
    const struct metal_f32_params *f32_params,
    ...
    return st != nullptr && residual != nullptr && out != nullptr &&
           f32_params != nullptr && post_params != nullptr &&
           st->use_ple_proj_norm_fused &&
           st->f32_ple_proj_norm_pipeline != nullptr &&
           f32_params->rows == post_params->rows &&
           f32_params->n_out == post_params->cols &&
           f32_params->y_offset == post_params->x_offset &&
           f32_params->y_row_stride == post_params->x_row_stride &&
           residual->buffer != out->buffer;
}

static void metal_encode_f32_ple_proj_norm(
    struct metal_state *st,
    void *enc,
    const struct geist_tensor *x,
    const struct geist_tensor *w,
    ...
    const struct metal_f32_ple_proj_norm_params params = {
        .n_in = f32_params->n_in,
        .n_out = f32_params->n_out,
        .rows = f32_params->rows,
        .x_offset = f32_params->x_offset,
        .w_offset = f32_params->w_offset,
        .residual_offset = post_params->residual_offset,
        .norm_weight_offset = post_params->w_offset,
        .y_offset = post_params->y_offset,
        .x_row_stride = f32_params->x_row_stride,
        .residual_row_stride = post_params->residual_row_stride,
        .y_row_stride = post_params->y_row_stride,
    ...
    metal_msg_send_set_buffer(st, enc, x->buffer->buffer, 0, 0);
    metal_msg_send_set_buffer(st, enc, w->buffer->buffer, 0, 1);
    metal_msg_send_set_buffer(st, enc, residual->buffer->buffer, 0, 2);
    metal_msg_send_set_buffer(st, enc, norm_weight->buffer->buffer, 0, 3);
    metal_msg_send_set_buffer(st, enc, y->buffer->buffer, 0, 4);
    metal_msg_send_set_bytes(st, enc, &params, sizeof(params), 5);

    const struct metal_size groups = {
        .width = f32_params->rows,
        .height = 1,
        .depth = 1,
    };
    const struct metal_size threads = {
    ...
    metal_profile_add_dispatch(st, METAL_PROFILE_DISPATCH_F32_PLE_PROJ_NORM,
                               groups);
    metal_msg_send_dispatch(st, enc, groups, threads);
}

static void metal_encode_argmax_f32(struct metal_state *st,
                                    void *enc,
                                    const struct geist_tensor *logits,
                                    const struct metal_argmax_params *params) {
    (void) metal_msg_send_id_id(st, enc, "setComputePipelineState:",
                                st->argmax_pipeline);
...
static void metal_encode_argmax_f32_batch(
    struct metal_state *st,
    void *enc,
    const struct geist_tensor *logits,
    const struct metal_argmax_batch_params *params) {
...
[[nodiscard]] static enum geist_status metal_argmax_f32(
    ...
    *out_token = -1;
    ...
    if (st->sequence_active) {
        geist_backend_set_error(
            be, GEIST_E_UNSUPPORTED,
            "metal argmax: captured standalone argmax is unsupported");
        return GEIST_E_UNSUPPORTED;
    }

    size_t n = 0;
    size_t offset = 0;
    if (!metal_tensor_is_f32_vector(logits, &n, &offset)) {
        geist_backend_set_error(
            be, GEIST_E_UNSUPPORTED,
            "metal argmax: expected F32 DENSE [n] or [1,n]");
        return GEIST_E_UNSUPPORTED;
    }
    if (n == 0 || n > UINT32_MAX || offset > UINT32_MAX ||
        logits->buffer->owner != st) {
        return GEIST_E_INVALID_ARG;
    }
    ...
    const struct metal_argmax_params params = {
        .n = (uint32_t) n,
        .x_offset = (uint32_t) offset,
    };
    uint32_t sentinel = UINT32_MAX;
    memcpy(st->argmax_result_mapped, &sentinel, sizeof(sentinel));
    ...
    if (cmd == nullptr || enc == nullptr) {
        geist_backend_set_error(be, GEIST_E_BACKEND,
                                "metal argmax: command encoder failed");
        return GEIST_E_BACKEND;
    }
    metal_encode_argmax_f32(st, enc, logits, &params);
    metal_msg_send_void0(st, enc, "endEncoding");
    metal_msg_send_void0(st, cmd, "commit");
    metal_msg_send_void0(st, cmd, "waitUntilCompleted");
    void *err = metal_msg_send_id0(st, cmd, "error");
    if (err != nullptr) {
        geist_backend_set_error(be, GEIST_E_BACKEND,
                                "metal argmax: command failed");
        return GEIST_E_BACKEND;
    }

    uint32_t token_u32 = UINT32_MAX;
    memcpy(&token_u32, st->argmax_result_mapped, sizeof(token_u32));
    if (token_u32 >= n) {
        geist_backend_set_error(be, GEIST_E_BACKEND,
                                "metal argmax: invalid token id %u",
                                token_u32);
        return GEIST_E_BACKEND;
    }
...
[[nodiscard]] static enum geist_status metal_argmax_f32_batch(
    struct geist_backend *be,
    const struct geist_tensor *logits,
    ...
    if (st->sequence_active) {
        geist_backend_set_error(
            be, GEIST_E_UNSUPPORTED,
            "metal argmax_batch: captured standalone argmax is unsupported");
        return GEIST_E_UNSUPPORTED;
    }
    ...
    size_t offset = 0;
    size_t row_stride = 0;
    if (!metal_tensor_is_f32_matrix(logits, &rows, &n, &offset,
                                    &row_stride)) {
        geist_backend_set_error(be, GEIST_E_UNSUPPORTED,
                                "metal argmax_batch: expected F32 DENSE [rows,n]");
        return GEIST_E_UNSUPPORTED;
    }
    if (rows == 0 || n == 0 || rows > UINT32_MAX ||
        n > UINT32_MAX || offset > UINT32_MAX ||
        row_stride > UINT32_MAX || logits->buffer->owner != st) {
        return GEIST_E_INVALID_ARG;
    }

    enum geist_status s =
        metal_ensure_argmax_result_capacity(be, (uint32_t) rows);
    if (s != GEIST_OK) { return s; }
    const struct metal_argmax_batch_params params = {
        .rows = (uint32_t) rows,
        .n = (uint32_t) n,
        .x_offset = (uint32_t) offset,
        .x_row_stride = (uint32_t) row_stride,
        .out_offset = 0,
    };
    for (size_t row = 0; row < rows; row++) {
        uint32_t sentinel = UINT32_MAX;
        memcpy((uint8_t *) st->argmax_result_mapped +
                   row * sizeof(uint32_t),
    ...
    if (cmd == nullptr || enc == nullptr) {
        geist_backend_set_error(be, GEIST_E_BACKEND,
                                "metal argmax_batch: command encoder failed");
        return GEIST_E_BACKEND;
    }
    metal_encode_argmax_f32_batch(st, enc, logits, &params);
    metal_msg_send_void0(st, enc, "endEncoding");
    metal_msg_send_void0(st, cmd, "commit");
    metal_msg_send_void0(st, cmd, "waitUntilCompleted");
    void *err = metal_msg_send_id0(st, cmd, "error");
    if (err != nullptr) {
        geist_backend_set_error(be, GEIST_E_BACKEND,
                                "metal argmax_batch: command failed");
        return GEIST_E_BACKEND;
    }
    ...
    for (size_t row = 0; row < rows; row++) {
        uint32_t token_u32 = UINT32_MAX;
        memcpy(&token_u32,
               (const uint8_t *) st->argmax_result_mapped +
                   row * sizeof(uint32_t),
               sizeof(token_u32));
        if (token_u32 >= n || token_u32 > (uint32_t) INT32_MAX) {
            geist_backend_set_error(be, GEIST_E_BACKEND,
                                    "metal argmax_batch: invalid token id %u",
                                    token_u32);
            return GEIST_E_BACKEND;
        }
...
[[nodiscard]] static enum geist_status metal_greedy_head(
    struct geist_backend *be,
    const struct geist_backend_greedy_head *head,
    geist_token_t *out_token) {
    ...
    *out_token = -1;
    ...
    size_t normed_n = 0;
    size_t normed_offset = 0;
    size_t logits_n = 0;
    size_t logits_offset = 0;
    if (!metal_tensor_is_f32_vector(head->hidden, &hidden_n,
                                    &hidden_offset) ||
        !metal_tensor_is_f32_vector(head->norm_weight, &norm_n,
                                    &norm_offset) ||
        !metal_tensor_is_f32_vector(head->normed_scratch, &normed_n,
                                    &normed_offset) ||
        !metal_tensor_is_f32_vector(head->logits, &logits_n,
                                    &logits_offset) ||
        hidden_n != head->d_model || norm_n != head->d_model ||
        normed_n != head->d_model || logits_n != head->vocab_size) {
        geist_backend_set_error(
            be, GEIST_E_UNSUPPORTED,
            "metal greedy_head: expected F32 DENSE hidden/norm/logits");
        return GEIST_E_UNSUPPORTED;
    }
    ...
    bool w_q4 = false;
    bool w_q6 = false;
    if (metal_tensor_is_f32_matrix(head->lm_head_weight, &w_rows, &w_cols,
                                   &w_offset, &w_stride) &&
        w_stride == w_cols) {
        w_f32 = true;
    } else if (metal_tensor_is_q4k_matrix(head->lm_head_weight, &w_rows,
                                          &w_cols, &w_offset)) {
        w_q4 = true;
    } else if (metal_tensor_is_q6k_matrix(head->lm_head_weight, &w_rows,
                                          &w_cols, &w_offset)) {
        w_q6 = true;
    }
    if ((!w_f32 && !w_q4 && !w_q6) ||
        w_rows != head->vocab_size || w_cols != head->d_model) {
        geist_backend_set_error(
            be, GEIST_E_UNSUPPORTED,
            "metal greedy_head: expected F32/Q4_K/Q6_K lm_head [vocab,d_model]");
        return GEIST_E_UNSUPPORTED;
    }

    struct metal_state *st = be->state;
    const struct geist_tensor *tensors[] = {
    ...
    for (size_t i = 0; i < sizeof(tensors) / sizeof(tensors[0]); i++) {
        if (tensors[i]->buffer->owner != st) {
            return GEIST_E_INVALID_ARG;
        }
    }
    if (hidden_offset > UINT32_MAX || norm_offset > UINT32_MAX ||
        normed_offset > UINT32_MAX || logits_offset > UINT32_MAX ||
        w_offset > UINT32_MAX) {
        return GEIST_E_INVALID_ARG;
    }
    ...
    s = metal_ensure_argmax_pipeline(be);
    if (s != GEIST_OK) {
        return s;
    }
    ...
    const struct metal_rows_params norm_params = {
        .rows = 1,
        .cols = (uint32_t) head->d_model,
        .x_offset = (uint32_t) hidden_offset,
        .w_offset = (uint32_t) norm_offset,
        .y_offset = (uint32_t) normed_offset,
        .x_row_stride = (uint32_t) head->d_model,
        .y_row_stride = (uint32_t) head->d_model,
        .eps = head->eps,
    };
    const struct metal_argmax_params argmax_params = {
        .n = (uint32_t) head->vocab_size,
        .x_offset = (uint32_t) logits_offset,
    };
    const struct metal_f32_params f32_params = {
        .n_in = (uint32_t) head->d_model,
        .n_out = (uint32_t) head->vocab_size,
        .rows = 1,
        .x_offset = (uint32_t) normed_offset,
        .w_offset = (uint32_t) w_offset,
        .y_offset = (uint32_t) logits_offset,
        .x_row_stride = (uint32_t) head->d_model,
        .y_row_stride = (uint32_t) head->vocab_size,
    };
    const struct metal_q4k_params q_params = {
        .n_in = (uint32_t) head->d_model,
        .n_out = (uint32_t) head->vocab_size,
        .rows = 1,
        .blocks_per_row =
            (uint32_t) (head->d_model /
                        (w_q6 ? METAL_Q6K_BLOCK_ELEMS
                              : METAL_Q4K_BLOCK_ELEMS)),
        .x_offset = (uint32_t) normed_offset,
    ...
    if (st->use_q4k_w4a8 && w_q4) {
        s = metal_ensure_q4k_w4a8_scratch(be, head->d_model);
        if (s != GEIST_OK) {
            return s;
        }
    }
    ...
    void *cmd = st->sequence_active
                    ? st->sequence_command_buffer
                    : metal_msg_send_id0(st, st->command_queue,
                                         "commandBuffer");
    void *enc = st->sequence_active
                    ? st->sequence_compute_encoder
                    : (cmd != nullptr
                           ? metal_msg_send_id0(st, cmd,
                                                "computeCommandEncoder")
                           : nullptr);
    if (cmd == nullptr || enc == nullptr) {
        geist_backend_set_error(be, GEIST_E_BACKEND,
                                "metal greedy_head: command encoder failed");
        return GEIST_E_BACKEND;
    }

    metal_encode_rmsnorm_rows(st, enc, head->hidden, head->norm_weight,
                              head->normed_scratch, &norm_params);
    if (w_f32) {
        metal_encode_f32_matmul(st, enc, head->normed_scratch,
                                head->lm_head_weight, head->logits,
                                &f32_params);
    } else if (w_q4) {
        metal_encode_q4k_linear(st, enc, head->normed_scratch,
                                head->lm_head_weight, head->logits,
                                &q_params, false);
    } else {
        metal_encode_q6k_linear(st, enc, head->normed_scratch,
                                head->lm_head_weight, head->logits,
                                &q_params, false);
    }
    metal_encode_argmax_f32(st, enc, head->logits, &argmax_params);

    if (st->sequence_active) {
        metal_decode_replay_record_greedy_head(st, head, w_f32, w_q4, w_q6,
                                               &norm_params, &f32_params,
                                               &q_params, &argmax_params);
        st->sequence_has_work = true;
        st->captured_greedy_token_pending = true;
        st->captured_greedy_vocab_size = (uint32_t) head->vocab_size;
        st->captured_greedy_token_count = 1u;
        return GEIST_OK;
    }
    ...
    if (err != nullptr) {
        const char *msg = metal_nserror_message(st, err);
        geist_backend_set_error(be, GEIST_E_BACKEND,
                                "metal greedy_head: command failed%s%s",
                                msg != nullptr ? ": " : "",
                                msg != nullptr ? msg : "");
        return GEIST_E_BACKEND;
    }
    ...
    uint32_t token_u32 = UINT32_MAX;
    memcpy(&token_u32, st->argmax_result_mapped, sizeof(token_u32));
    if (token_u32 >= head->vocab_size || token_u32 > (uint32_t) INT32_MAX) {
        geist_backend_set_error(be, GEIST_E_BACKEND,
                                "metal greedy_head: invalid token id %u",
                                token_u32);
        return GEIST_E_BACKEND;
    }
    *out_token = (geist_token_t) token_u32;
...
[[nodiscard]] static enum geist_status metal_greedy_head_batch(
    struct geist_backend *be,
    const struct geist_backend_greedy_head_batch *head,
    geist_token_t out_tokens[static head->row_count]) {

    if (be == nullptr || be->state == nullptr || head == nullptr ||
        head->struct_size < sizeof(*head) || out_tokens == nullptr ||
        head->hidden == nullptr || head->norm_weight == nullptr ||
        head->lm_head_weight == nullptr || head->normed_scratch == nullptr ||
        head->logits == nullptr) {
        return GEIST_E_INVALID_ARG;
    ...
    if (head->d_model == 0 || head->vocab_size == 0 ||
        head->row_count == 0 || head->d_model > UINT32_MAX ||
        head->vocab_size > UINT32_MAX || head->row_count > UINT32_MAX ||
        head->vocab_size > (size_t) INT32_MAX ||
        head->token_output_offset != 0) {
        return GEIST_E_INVALID_ARG;
    }
    ...
    size_t hidden_cols = 0;
    size_t hidden_offset = 0;
    size_t hidden_stride = 0;
    size_t norm_n = 0;
    size_t norm_offset = 0;
    size_t normed_rows = 0;
    size_t normed_cols = 0;
    size_t normed_offset = 0;
    size_t normed_stride = 0;
    size_t logits_rows = 0;
    size_t logits_cols = 0;
    size_t logits_offset = 0;
    size_t logits_stride = 0;
    if (!metal_tensor_is_f32_matrix(head->hidden, &hidden_rows,
                                    &hidden_cols, &hidden_offset,
                                    &hidden_stride) ||
        !metal_tensor_is_f32_vector(head->norm_weight, &norm_n,
                                    &norm_offset) ||
        !metal_tensor_is_f32_matrix(head->normed_scratch, &normed_rows,
                                    &normed_cols, &normed_offset,
                                    &normed_stride) ||
        !metal_tensor_is_f32_matrix(head->logits, &logits_rows,
                                    &logits_cols, &logits_offset,
                                    &logits_stride) ||
        hidden_rows != head->row_count ||
        normed_rows != head->row_count ||
        logits_rows != head->row_count ||
        hidden_cols != head->d_model ||
        norm_n != head->d_model ||
        normed_cols != head->d_model ||
        logits_cols != head->vocab_size) {
        geist_backend_set_error(
            be, GEIST_E_UNSUPPORTED,
            "metal greedy_head_batch: expected F32 DENSE batch tensors");
        return GEIST_E_UNSUPPORTED;
    }
    ...
    if ((!w_f32 && !w_q4 && !w_q6) ||
        ...
        geist_backend_set_error(
            be, GEIST_E_UNSUPPORTED,
            "metal greedy_head_batch: expected F32/Q4_K/Q6_K lm_head [vocab,d_model]");
        return GEIST_E_UNSUPPORTED;
    }
    ...
    if (hidden_offset > UINT32_MAX || hidden_stride > UINT32_MAX ||
        norm_offset > UINT32_MAX || normed_offset > UINT32_MAX ||
        normed_stride > UINT32_MAX || logits_offset > UINT32_MAX ||
        logits_stride > UINT32_MAX || w_offset > UINT32_MAX) {
        return GEIST_E_INVALID_ARG;
    }

    enum geist_status s = metal_ensure_q4k_pipeline(be);
    if (s != GEIST_OK) { return s; }
    s = metal_ensure_argmax_result_capacity(be, (uint32_t) head->row_count);
    if (s != GEIST_OK) { return s; }

    const struct metal_rows_params norm_params = {
        .rows = (uint32_t) head->row_count,
        .cols = (uint32_t) head->d_model,
        .x_offset = (uint32_t) hidden_offset,
        .w_offset = (uint32_t) norm_offset,
        .y_offset = (uint32_t) normed_offset,
        .x_row_stride = (uint32_t) hidden_stride,
        .y_row_stride = (uint32_t) normed_stride,
        .eps = head->eps,
    };
    const struct metal_f32_params f32_params = {
        .n_in = (uint32_t) head->d_model,
        ...
        .w_offset = (uint32_t) w_offset,
        .y_offset = (uint32_t) logits_offset,
        .x_row_stride = (uint32_t) normed_stride,
        .y_row_stride = (uint32_t) logits_stride,
    };
    const struct metal_q4k_params q_params = {
        .n_in = (uint32_t) head->d_model,
        .n_out = (uint32_t) head->vocab_size,
    ...
    const struct metal_argmax_batch_params argmax_params = {
        .rows = (uint32_t) head->row_count,
        .n = (uint32_t) head->vocab_size,
        .x_offset = (uint32_t) logits_offset,
        .x_row_stride = (uint32_t) logits_stride,
        .out_offset = 0,
    };
    for (size_t row = 0; row < head->row_count; row++) {
        uint32_t sentinel = UINT32_MAX;
    ...
    if (cmd == nullptr || enc == nullptr) {
        geist_backend_set_error(be, GEIST_E_BACKEND,
                                "metal greedy_head_batch: command encoder failed");
        return GEIST_E_BACKEND;
    }
    ...
    if (w_f32) {
        ...
        metal_encode_q4k_linear(st, enc, head->normed_scratch,
                                head->lm_head_weight, head->logits,
                                &q_params, head->row_count > 1u);
    } else {
        metal_encode_q6k_linear(st, enc, head->normed_scratch,
                                head->lm_head_weight, head->logits,
                                &q_params, head->row_count > 1u);
    }
    metal_encode_argmax_f32_batch(st, enc, head->logits, &argmax_params);

    if (st->sequence_active) {
        st->sequence_has_work = true;
        st->captured_greedy_token_pending = true;
        st->captured_greedy_vocab_size = (uint32_t) head->vocab_size;
        st->captured_greedy_token_count = (uint32_t) head->row_count;
        return GEIST_OK;
    }
    ...
    if (err != nullptr) {
        const char *msg = metal_nserror_message(st, err);
        geist_backend_set_error(be, GEIST_E_BACKEND,
                                "metal greedy_head_batch: command failed%s%s",
                                msg != nullptr ? ": " : "",
                                msg != nullptr ? msg : "");
        return GEIST_E_BACKEND;
    }
    ...
    for (size_t row = 0; row < head->row_count; row++) {
        ...
        if (token_u32 >= head->vocab_size ||
            ...
            geist_backend_set_error(
                be, GEIST_E_BACKEND,
                "metal greedy_head_batch: invalid token id %u", token_u32);
            return GEIST_E_BACKEND;
        }
        out_tokens[row] = (geist_token_t) token_u32;
    }
...
static bool metal_tensor_owned_by(const struct geist_tensor *t,
                                  const struct metal_state *st) {
    return t != nullptr && t->buffer != nullptr && t->buffer->owner == st;
}
...
[[nodiscard]] static enum geist_status metal_ple_block(
    ...
    const struct metal_state *state_cfg = be->state;
    if (!state_cfg->use_ple_block && !state_cfg->sequence_active) {
        return GEIST_E_UNSUPPORTED;
    }
    if (block->seq == 0 || block->d_model == 0 ||
        block->hidden_per_layer == 0 || block->hidden == nullptr ||
        block->per_layer_input == nullptr ||
        block->per_layer_gate_weight == nullptr ||
        block->per_layer_proj_weight == nullptr ||
        block->post_per_layer_norm_weight == nullptr ||
        block->gate_scratch == nullptr ||
        block->proj_scratch == nullptr ||
        block->out == nullptr) {
        return GEIST_E_INVALID_ARG;
    }

    size_t hidden_rows = 0, hidden_cols = 0, hidden_offset = 0;
    size_t hidden_stride = 0;
    size_t ple_rows = 0, ple_cols = 0, ple_offset = 0;
    size_t ple_stride = 0;
    size_t gate_rows = 0, gate_cols = 0, gate_offset = 0;
    size_t gate_stride = 0;
    size_t proj_rows = 0, proj_cols = 0, proj_offset = 0;
    size_t proj_stride = 0;
    size_t out_rows = 0, out_cols = 0, out_offset = 0;
    size_t out_stride = 0;
    size_t norm_n = 0, norm_offset = 0;
    size_t gate_w_rows = 0, gate_w_cols = 0, gate_w_offset = 0;
    size_t proj_w_rows = 0, proj_w_cols = 0, proj_w_offset = 0;
    size_t gate_w_stride = 0, proj_w_stride = 0;
    bool gate_w_q4 = metal_tensor_is_q4k_matrix(
        block->per_layer_gate_weight, &gate_w_rows, &gate_w_cols,
        &gate_w_offset);
    bool proj_w_q4 = metal_tensor_is_q4k_matrix(
        block->per_layer_proj_weight, &proj_w_rows, &proj_w_cols,
        &proj_w_offset);
    if (!gate_w_q4) {
        gate_w_q4 = false;
        if (!metal_tensor_is_f32_matrix(block->per_layer_gate_weight,
                                        &gate_w_rows, &gate_w_cols,
                                        &gate_w_offset, &gate_w_stride)) {
            geist_backend_set_error(
                be, GEIST_E_UNSUPPORTED,
                "metal PLE block: per-layer gate weight unsupported");
            return GEIST_E_UNSUPPORTED;
        }
    }
    if (!proj_w_q4) {
        proj_w_q4 = false;
        if (!metal_tensor_is_f32_matrix(block->per_layer_proj_weight,
                                        &proj_w_rows, &proj_w_cols,
                                        &proj_w_offset, &proj_w_stride)) {
            geist_backend_set_error(
                be, GEIST_E_UNSUPPORTED,
                "metal PLE block: per-layer projection weight unsupported");
            return GEIST_E_UNSUPPORTED;
        }
    }

    if (!metal_tensor_is_f32_matrix(block->hidden, &hidden_rows,
                                    &hidden_cols, &hidden_offset,
                                    &hidden_stride) ||
        !metal_tensor_is_f32_matrix_strided(block->per_layer_input, &ple_rows,
                                            &ple_cols, &ple_offset,
                                            &ple_stride) ||
        !metal_tensor_is_f32_matrix(block->gate_scratch, &gate_rows,
                                    &gate_cols, &gate_offset, &gate_stride) ||
        !metal_tensor_is_f32_matrix(block->proj_scratch, &proj_rows,
                                    &proj_cols, &proj_offset, &proj_stride) ||
        !metal_tensor_is_f32_matrix(block->out, &out_rows, &out_cols,
                                    &out_offset, &out_stride) ||
        !metal_tensor_is_f32_vector(block->post_per_layer_norm_weight,
                                    &norm_n, &norm_offset)) {
        geist_backend_set_error(be, GEIST_E_UNSUPPORTED,
                                "metal PLE block: unsupported tensor layout");
        return GEIST_E_UNSUPPORTED;
    }

    const size_t rows = block->seq;
    const size_t d_model = block->d_model;
    const size_t hpl = block->hidden_per_layer;
    if (hidden_rows != rows || ple_rows != rows || gate_rows != rows ||
        proj_rows != rows || out_rows != rows ||
        hidden_cols != d_model || proj_cols != d_model ||
        out_cols != d_model || ple_cols != hpl || gate_cols != hpl ||
        norm_n != d_model ||
        gate_w_rows != hpl || gate_w_cols != d_model ||
        proj_w_rows != d_model || proj_w_cols != hpl ||
        (!gate_w_q4 && gate_w_stride != d_model) ||
        (!proj_w_q4 && proj_w_stride != hpl)) {
        geist_backend_set_error(be, GEIST_E_UNSUPPORTED,
                                "metal PLE block: shape mismatch");
        return GEIST_E_UNSUPPORTED;
    }
    if (rows > UINT32_MAX || d_model > UINT32_MAX || hpl > UINT32_MAX ||
        rows > UINT32_MAX / d_model || rows > UINT32_MAX / hpl ||
        hidden_offset > UINT32_MAX || ple_offset > UINT32_MAX ||
        gate_offset > UINT32_MAX || proj_offset > UINT32_MAX ||
        out_offset > UINT32_MAX || norm_offset > UINT32_MAX ||
        gate_w_offset > UINT32_MAX || proj_w_offset > UINT32_MAX ||
        hidden_stride > UINT32_MAX || ple_stride > UINT32_MAX ||
        gate_stride > UINT32_MAX || proj_stride > UINT32_MAX ||
        out_stride > UINT32_MAX) {
        return GEIST_E_INVALID_ARG;
    }

    struct metal_state *st = be->state;
    const struct geist_tensor *owned[] = {
        block->hidden, block->per_layer_input,
        block->per_layer_gate_weight, block->per_layer_proj_weight,
        block->post_per_layer_norm_weight, block->gate_scratch,
        block->proj_scratch, block->out,
    };
    for (size_t i = 0; i < sizeof(owned) / sizeof(owned[0]); i++) {
        if (!metal_tensor_owned_by(owned[i], st)) {
            return GEIST_E_INVALID_ARG;
        }
    }
    ...
    if (st->sequence_active) {
        if (st->sequence_compute_encoder == nullptr) {
            geist_backend_set_error(be, GEIST_E_BACKEND,
                                    "metal PLE block: sequence has no encoder");
            return GEIST_E_BACKEND;
        }
        ...
        if (cmd == nullptr || enc == nullptr) {
            geist_backend_set_error(be, GEIST_E_BACKEND,
                                    "metal PLE block: command encoder failed");
            return GEIST_E_BACKEND;
        }
    }
    ...
    const struct metal_q4k_params gate_q4_params = {
        ...
        .n_out = (uint32_t) hpl,
        .rows = (uint32_t) rows,
        .blocks_per_row = (uint32_t) (d_model / METAL_Q4K_BLOCK_ELEMS),
        .x_offset = (uint32_t) hidden_offset,
        .w_byte_offset = (uint32_t) gate_w_offset,
    ...
    const struct metal_q4k_params proj_q4_params = {
        .n_in = (uint32_t) hpl,
        .n_out = (uint32_t) d_model,
        .rows = (uint32_t) rows,
        .blocks_per_row = (uint32_t) (hpl / METAL_Q4K_BLOCK_ELEMS),
        .x_offset = (uint32_t) gate_offset,
        .w_byte_offset = (uint32_t) proj_w_offset,
        .y_offset = (uint32_t) proj_offset,
        .x_row_stride = (uint32_t) gate_stride,
        .y_row_stride = (uint32_t) proj_stride,
    };
    const struct metal_f32_params gate_f32_params = {
        .n_in = (uint32_t) d_model,
        .n_out = (uint32_t) hpl,
        .rows = (uint32_t) rows,
        .x_offset = (uint32_t) hidden_offset,
        .w_offset = (uint32_t) gate_w_offset,
        .y_offset = (uint32_t) gate_offset,
        .x_row_stride = (uint32_t) hidden_stride,
        .y_row_stride = (uint32_t) gate_stride,
    };
    const struct metal_f32_params proj_f32_params = {
        ...
        .rows = (uint32_t) rows,
        .x_offset = (uint32_t) gate_offset,
        .w_offset = (uint32_t) proj_w_offset,
        .y_offset = (uint32_t) proj_offset,
        .x_row_stride = (uint32_t) gate_stride,
        .y_row_stride = (uint32_t) proj_stride,
    };
    const struct metal_binary_rows_params act_params = {
        ...
        .b_offset = (uint32_t) ple_offset,
        .y_offset = (uint32_t) gate_offset,
        .a_row_stride = (uint32_t) gate_stride,
        .b_row_stride = (uint32_t) ple_stride,
        .y_row_stride = (uint32_t) gate_stride,
    };
    const struct metal_post_norm_params post_params = {
        .rows = (uint32_t) rows,
        .cols = (uint32_t) d_model,
        .residual_offset = (uint32_t) hidden_offset,
        .x_offset = (uint32_t) proj_offset,
        .w_offset = (uint32_t) norm_offset,
        .y_offset = (uint32_t) out_offset,
        .residual_row_stride = (uint32_t) hidden_stride,
        .x_row_stride = (uint32_t) proj_stride,
        .y_row_stride = (uint32_t) out_stride,
        .eps = block->eps,
    };

    if (st->sequence_active) {
        metal_decode_replay_record_ple(
            st, block, gate_w_q4, proj_w_q4, &gate_q4_params,
            &proj_q4_params, &gate_f32_params, &proj_f32_params,
            &act_params, &post_params);
    }

    bool fused_gate =
        gate_w_q4 &&
        metal_try_encode_q4k_ple_gate_nt8(
            st, enc, block->hidden, block->per_layer_gate_weight,
            block->per_layer_input, block->gate_scratch, &gate_q4_params,
            &act_params);
    if (!fused_gate && gate_w_q4) {
        metal_encode_q4k_linear(st, enc, block->hidden,
        ...
        metal_encode_f32_ple_gate(st, enc, block->hidden,
                                  ...
                                  block->gate_scratch, &gate_f32_params,
                                  &act_params);
        fused_gate = true;
    } else if (!fused_gate) {
        metal_encode_f32_matmul(st, enc, block->hidden,
                                block->per_layer_gate_weight,
                                block->gate_scratch, &gate_f32_params);
    }
    if (!fused_gate) {
    ...
    if (!proj_w_q4 &&
        metal_can_encode_f32_ple_proj_norm(st, block->hidden, block->out,
                                           &proj_f32_params, &post_params)) {
        metal_encode_f32_ple_proj_norm(st, enc, block->gate_scratch,
                                       block->per_layer_proj_weight,
                                       block->hidden,
                                       block->post_per_layer_norm_weight,
                                       block->out, &proj_f32_params,
                                       &post_params);
    } else if (proj_w_q4) {
        metal_encode_q4k_linear(st, enc, block->gate_scratch,
                                block->per_layer_proj_weight,
                                block->proj_scratch, &proj_q4_params,
                                rows > 1);
        metal_encode_rmsnorm_add_rows(st, enc, block->hidden,
                                      block->proj_scratch,
                                      block->post_per_layer_norm_weight,
                                      block->out, &post_params);
    } else {
        metal_encode_f32_matmul(st, enc, block->gate_scratch,
                                block->per_layer_proj_weight,
                                block->proj_scratch, &proj_f32_params);
        metal_encode_rmsnorm_add_rows(st, enc, block->hidden,
                                      block->proj_scratch,
                                      block->post_per_layer_norm_weight,
                                      block->out, &post_params);
    ...
    if (err != nullptr) {
        const char *msg = metal_nserror_message(st, err);
        geist_backend_set_error(be, GEIST_E_BACKEND,
                                "metal PLE block: command failed%s%s",
                                msg != nullptr ? ": " : "",
                                msg != nullptr ? msg : "");
        return GEIST_E_BACKEND;
    }
...
[[nodiscard]] static enum geist_status metal_ffn_geglu_block(
    ...
    if (block->seq == 0 || block->d_model == 0 || block->inter == 0 ||
        block->post_ffw_norm_weight == nullptr ||
        block->post_ff_scratch == nullptr) {
        geist_backend_set_error(
            be, GEIST_E_UNSUPPORTED,
            "metal FFN GEGLU block: expected post-norm Gemma block");
        return GEIST_E_UNSUPPORTED;
    }

    size_t residual_rows = 0;
    size_t residual_cols = 0;
    size_t residual_offset = 0;
    size_t residual_stride = 0;
    size_t pre_rows = 0;
    size_t pre_cols = 0;
    size_t pre_offset = 0;
    size_t pre_stride = 0;
    ...
    size_t up_offset = 0;
    size_t up_stride = 0;
    size_t ffn_out_rows = 0;
    size_t ffn_out_cols = 0;
    size_t ffn_out_offset = 0;
    size_t ffn_out_stride = 0;
    size_t post_rows = 0;
    size_t post_cols = 0;
    size_t post_offset = 0;
    size_t post_stride = 0;
    size_t out_rows = 0;
    size_t out_cols = 0;
    size_t out_offset = 0;
    size_t out_stride = 0;
    size_t ffn_norm_n = 0;
    size_t ffn_norm_offset = 0;
    size_t post_norm_n = 0;
    size_t post_norm_offset = 0;
    size_t gate_w_rows = 0;
    size_t gate_w_cols = 0;
    size_t gate_w_offset = 0;
    size_t up_w_rows = 0;
    size_t up_w_cols = 0;
    size_t up_w_offset = 0;
    size_t down_w_rows = 0;
    size_t down_w_cols = 0;
    size_t down_w_offset = 0;
    bool down_w_q6 = false;
    bool down_w_ok = metal_tensor_is_q4k_matrix(
        block->down_weight, &down_w_rows, &down_w_cols, &down_w_offset);
    if (!down_w_ok) {
        down_w_ok = metal_tensor_is_q6k_matrix(
            block->down_weight, &down_w_rows, &down_w_cols, &down_w_offset);
        down_w_q6 = down_w_ok;
    }

    if (!metal_tensor_is_f32_matrix(block->residual, &residual_rows,
                                    &residual_cols, &residual_offset,
                                    &residual_stride) ||
        !metal_tensor_is_f32_matrix(block->pre_ff_scratch, &pre_rows,
                                    &pre_cols, &pre_offset, &pre_stride) ||
        !metal_tensor_is_f32_matrix(block->gate_scratch, &gate_rows,
                                    &gate_cols, &gate_offset, &gate_stride) ||
        !metal_tensor_is_f32_matrix(block->up_scratch, &up_rows, &up_cols,
                                    &up_offset, &up_stride) ||
        !metal_tensor_is_f32_matrix(block->ffn_out_scratch, &ffn_out_rows,
                                    &ffn_out_cols, &ffn_out_offset,
                                    &ffn_out_stride) ||
        !metal_tensor_is_f32_matrix(block->post_ff_scratch, &post_rows,
                                    &post_cols, &post_offset, &post_stride) ||
        !metal_tensor_is_f32_matrix(block->out, &out_rows, &out_cols,
                                    &out_offset, &out_stride) ||
        !metal_tensor_is_f32_vector(block->ffn_norm_weight, &ffn_norm_n,
                                    &ffn_norm_offset) ||
        !metal_tensor_is_f32_vector(block->post_ffw_norm_weight,
                                    &post_norm_n, &post_norm_offset) ||
        !metal_tensor_is_q4k_matrix(block->gate_weight, &gate_w_rows,
                                    &gate_w_cols, &gate_w_offset) ||
        !metal_tensor_is_q4k_matrix(block->up_weight, &up_w_rows,
                                    &up_w_cols, &up_w_offset) ||
        !down_w_ok) {
        geist_backend_set_error(
            be, GEIST_E_UNSUPPORTED,
            "metal FFN GEGLU block: unsupported tensor layout");
        return GEIST_E_UNSUPPORTED;
    }
    ...
    if (residual_rows != rows || pre_rows != rows || gate_rows != rows ||
        up_rows != rows || ffn_out_rows != rows || post_rows != rows ||
        out_rows != rows || residual_cols != d_model ||
        pre_cols != d_model || ffn_out_cols != d_model ||
        post_cols != d_model || out_cols != d_model ||
        gate_cols != inter || up_cols != inter ||
        ffn_norm_n != d_model || post_norm_n != d_model ||
        gate_w_rows != inter || gate_w_cols != d_model ||
        up_w_rows != inter || up_w_cols != d_model ||
        down_w_rows != d_model || down_w_cols != inter) {
        geist_backend_set_error(
            be, GEIST_E_UNSUPPORTED,
            "metal FFN GEGLU block: shape mismatch");
        return GEIST_E_UNSUPPORTED;
    }
    if (rows > UINT32_MAX || d_model > UINT32_MAX ||
        inter > UINT32_MAX || rows > UINT32_MAX / d_model ||
        rows > UINT32_MAX / inter || residual_offset > UINT32_MAX ||
        pre_offset > UINT32_MAX || gate_offset > UINT32_MAX ||
        up_offset > UINT32_MAX || ffn_out_offset > UINT32_MAX ||
        post_offset > UINT32_MAX || out_offset > UINT32_MAX ||
        ffn_norm_offset > UINT32_MAX || post_norm_offset > UINT32_MAX ||
        gate_w_offset > UINT32_MAX || up_w_offset > UINT32_MAX ||
        down_w_offset > UINT32_MAX || residual_stride > UINT32_MAX ||
        pre_stride > UINT32_MAX || gate_stride > UINT32_MAX ||
        up_stride > UINT32_MAX || ffn_out_stride > UINT32_MAX ||
        post_stride > UINT32_MAX || out_stride > UINT32_MAX) {
        geist_backend_set_error(be, GEIST_E_INVALID_ARG,
                                "metal FFN GEGLU block: tensor too large");
        return GEIST_E_INVALID_ARG;
    }

    struct metal_state *st = be->state;
    if (!metal_tensor_owned_by(block->residual, st) ||
        !metal_tensor_owned_by(block->ffn_norm_weight, st) ||
        !metal_tensor_owned_by(block->gate_weight, st) ||
        !metal_tensor_owned_by(block->up_weight, st) ||
        !metal_tensor_owned_by(block->down_weight, st) ||
        !metal_tensor_owned_by(block->post_ffw_norm_weight, st) ||
        !metal_tensor_owned_by(block->pre_ff_scratch, st) ||
        !metal_tensor_owned_by(block->gate_scratch, st) ||
        !metal_tensor_owned_by(block->up_scratch, st) ||
        !metal_tensor_owned_by(block->ffn_out_scratch, st) ||
        !metal_tensor_owned_by(block->post_ff_scratch, st) ||
        !metal_tensor_owned_by(block->out, st)) {
        return GEIST_E_INVALID_ARG;
    }
    ...
    s = metal_ensure_q4k_gate_up_pipeline(be);
    if (s != GEIST_OK) {
        return s;
    }
    if (st->use_q4k_w4a8 && rows == 1u) {
        size_t w4a8_n_in = d_model;
        if (!down_w_q6 && inter > w4a8_n_in) {
            w4a8_n_in = inter;
        }
        s = metal_ensure_q4k_w4a8_scratch(be, w4a8_n_in);
        if (s != GEIST_OK) {
            return s;
        }
    }
    ...
    if (st->sequence_active) {
        if (st->sequence_compute_encoder == nullptr) {
            geist_backend_set_error(
                be, GEIST_E_BACKEND,
                "metal FFN GEGLU block: command sequence has no encoder");
            return GEIST_E_BACKEND;
        }
        enc = st->sequence_compute_encoder;
    } else {
        ...
        if (cmd == nullptr) {
            geist_backend_set_error(
                be, GEIST_E_BACKEND,
                "metal FFN GEGLU block: command buffer failed");
            return GEIST_E_BACKEND;
        }
        enc = metal_msg_send_id0(st, cmd, "computeCommandEncoder");
        if (enc == nullptr) {
            geist_backend_set_error(be, GEIST_E_BACKEND,
                                    "metal FFN GEGLU block: encoder failed");
            return GEIST_E_BACKEND;
        }
    }
    ...
    const struct metal_rows_params pre_norm_params = {
        .rows = (uint32_t) rows,
        .cols = (uint32_t) d_model,
        .x_offset = (uint32_t) residual_offset,
        .w_offset = (uint32_t) ffn_norm_offset,
        .y_offset = (uint32_t) pre_offset,
        .x_row_stride = (uint32_t) residual_stride,
        .y_row_stride = (uint32_t) pre_stride,
        .eps = block->eps,
    };
    const struct metal_q4k_params gate_params = {
        .n_in = (uint32_t) d_model,
        .n_out = (uint32_t) inter,
        .rows = (uint32_t) rows,
        .blocks_per_row = (uint32_t) (d_model / METAL_Q4K_BLOCK_ELEMS),
        .x_offset = (uint32_t) pre_offset,
        .w_byte_offset = (uint32_t) gate_w_offset,
        .y_offset = (uint32_t) gate_offset,
        .x_row_stride = (uint32_t) pre_stride,
        .y_row_stride = (uint32_t) gate_stride,
    };
    const struct metal_q4k_params up_params = {
        .n_in = (uint32_t) d_model,
        .n_out = (uint32_t) inter,
        .rows = (uint32_t) rows,
        .blocks_per_row = (uint32_t) (d_model / METAL_Q4K_BLOCK_ELEMS),
        .x_offset = (uint32_t) pre_offset,
        .w_byte_offset = (uint32_t) up_w_offset,
        .y_offset = (uint32_t) up_offset,
        .x_row_stride = (uint32_t) pre_stride,
        .y_row_stride = (uint32_t) up_stride,
    };
    const struct metal_q4k_gate_up_params gate_up_params = {
        .n_in = (uint32_t) d_model,
        .n_out = (uint32_t) inter,
        ...
        .blocks_per_row = (uint32_t) (d_model / METAL_Q4K_BLOCK_ELEMS),
        .x_offset = (uint32_t) pre_offset,
        .gate_w_byte_offset = (uint32_t) gate_w_offset,
        .up_w_byte_offset = (uint32_t) up_w_offset,
        .gate_y_offset = (uint32_t) gate_offset,
        .up_y_offset = (uint32_t) up_offset,
        .x_row_stride = (uint32_t) pre_stride,
        .y_row_stride = (uint32_t) gate_stride,
    };
    const struct metal_q4k_params down_params = {
        ...
        .rows = (uint32_t) rows,
        .blocks_per_row =
            (uint32_t) (inter / (down_w_q6 ? METAL_Q6K_BLOCK_ELEMS
                                           : METAL_Q4K_BLOCK_ELEMS)),
        .x_offset = (uint32_t) gate_offset,
        .w_byte_offset = (uint32_t) down_w_offset,
        .y_offset = (uint32_t) ffn_out_offset,
        .x_row_stride = (uint32_t) gate_stride,
        .y_row_stride = (uint32_t) ffn_out_stride,
    };
    const struct metal_post_norm_params post_params = {
        .rows = (uint32_t) rows,
        .cols = (uint32_t) d_model,
        .residual_offset = (uint32_t) residual_offset,
        .x_offset = (uint32_t) ffn_out_offset,
        .w_offset = (uint32_t) post_norm_offset,
        .y_offset = (uint32_t) out_offset,
        .residual_row_stride = (uint32_t) residual_stride,
        .x_row_stride = (uint32_t) ffn_out_stride,
        .y_row_stride = (uint32_t) out_stride,
        .eps = block->eps,
    };

    if (st->sequence_active) {
        metal_decode_replay_record_ffn(st, block, down_w_q6,
                                       &pre_norm_params, &gate_up_params,
    ...
    metal_encode_rmsnorm_add_rows(st, enc, block->residual,
                                  block->ffn_out_scratch,
                                  block->post_ffw_norm_weight, block->out,
                                  &post_params);
    ...
    metal_msg_send_void0(st, cmd, "commit");
    metal_msg_send_void0(st, cmd, "waitUntilCompleted");
    metal_profile_add_wait(st, METAL_PROFILE_WAIT_FFN_STANDALONE,
                           wait_start_ns);

    void *err = metal_msg_send_id0(st, cmd, "error");
    if (err != nullptr) {
        geist_backend_set_error(be, GEIST_E_BACKEND,
                                "metal FFN GEGLU block: command failed");
        return GEIST_E_BACKEND;
    }
    return GEIST_OK;
}

[[nodiscard]] static enum geist_status metal_attention_block(
    struct geist_backend *be,
    const struct geist_backend_attention_block *block) {
    ...
    if (block->d_model == 0 || block->q_heads == 0 ||
        block->kv_heads == 0 || block->head_dim == 0 ||
    ...
    size_t norm_offset = 0;
    size_t q_norm_n = 0;
    size_t q_norm_offset = 0;
    size_t k_norm_n = 0;
    size_t k_norm_offset = 0;
    size_t v_norm_n = 0;
    size_t v_norm_offset = 0;
    size_t post_norm_n = 0;
    size_t post_norm_offset = 0;
    size_t cos_rows = 0;
    size_t cos_cols = 0;
    ...
    size_t sin_offset = 0;
    size_t sin_stride = 0;
    size_t k_cache_len = 0;
    size_t k_cache_heads = 0;
    size_t k_cache_hd = 0;
    size_t k_cache_offset = 0;
    size_t v_cache_len = 0;
    size_t v_cache_heads = 0;
    size_t v_cache_hd = 0;
    ...
    size_t o_w_offset = 0;
    bool v_w_q6 = false;
    bool v_w_ok = metal_tensor_is_q4k_matrix(
        block->v_proj_weight, &v_w_rows, &v_w_cols, &v_w_offset);
    if (!v_w_ok) {
        v_w_ok = metal_tensor_is_q6k_matrix(
            block->v_proj_weight, &v_w_rows, &v_w_cols, &v_w_offset);
        v_w_q6 = v_w_ok;
    }
    const bool kv_cache_f32 =
        metal_tensor_is_f32_3d(block->k_cache, &k_cache_len,
                               &k_cache_heads, &k_cache_hd,
                               &k_cache_offset) &&
        metal_tensor_is_f32_3d(block->v_cache, &v_cache_len,
                               &v_cache_heads, &v_cache_hd,
                               &v_cache_offset);
    const bool kv_cache_f16 =
        !kv_cache_f32 &&
        metal_tensor_is_f16_3d(block->k_cache, &k_cache_len,
                               &k_cache_heads, &k_cache_hd,
                               &k_cache_offset) &&
        metal_tensor_is_f16_3d(block->v_cache, &v_cache_len,
                               &v_cache_heads, &v_cache_hd,
                               &v_cache_offset);
    ...
    if (!metal_tensor_is_f32_matrix(block->residual, &residual_rows,
                                    &residual_cols, &residual_offset,
                                    &residual_stride) ||
        !metal_tensor_is_f32_matrix(block->normed_scratch, &normed_rows,
                                    &normed_cols, &normed_offset,
                                    &normed_stride) ||
        !metal_tensor_is_f32_matrix(block->q_scratch, &q_rows, &q_cols,
                                    &q_offset, &q_stride) ||
        !metal_tensor_is_f32_matrix(block->k_scratch, &k_rows, &k_cols,
                                    &k_offset, &k_stride) ||
        !metal_tensor_is_f32_matrix(block->v_scratch, &v_rows, &v_cols,
                                    &v_offset, &v_stride) ||
        !metal_tensor_is_f32_matrix(block->attn_scratch, &attn_rows,
                                    &attn_cols, &attn_offset,
                                    &attn_stride) ||
        !metal_tensor_is_f32_matrix(block->o_scratch, &o_rows, &o_cols,
                                    &o_offset, &o_stride) ||
        !metal_tensor_is_f32_matrix(block->out, &out_rows, &out_cols,
                                    &out_offset, &out_stride) ||
        !metal_tensor_is_f32_vector(block->attn_norm_weight, &norm_n,
                                    &norm_offset) ||
        !metal_tensor_is_f32_vector(block->q_norm_weight, &q_norm_n,
                                    &q_norm_offset) ||
        !metal_tensor_is_f32_vector(block->k_norm_weight, &k_norm_n,
                                    &k_norm_offset) ||
        !metal_tensor_is_f32_vector(block->v_norm_weight, &v_norm_n,
                                    &v_norm_offset) ||
        !metal_tensor_is_f32_vector(block->post_attn_norm_weight,
                                    &post_norm_n, &post_norm_offset) ||
        !metal_tensor_is_f32_matrix(block->cos, &cos_rows, &cos_cols,
                                    &cos_offset, &cos_stride) ||
        !metal_tensor_is_f32_matrix(block->sin, &sin_rows, &sin_cols,
                                    &sin_offset, &sin_stride) ||
        (!kv_cache_f32 && !kv_cache_f16) ||
        !metal_tensor_is_q4k_matrix(block->q_proj_weight, &q_w_rows,
                                    &q_w_cols, &q_w_offset) ||
        !metal_tensor_is_q4k_matrix(block->k_proj_weight, &k_w_rows,
                                    &k_w_cols, &k_w_offset) ||
        !v_w_ok ||
        !metal_tensor_is_q4k_matrix(block->o_proj_weight, &o_w_rows,
                                    &o_w_cols, &o_w_offset)) {
        geist_backend_set_error(
            be, GEIST_E_UNSUPPORTED,
            "metal attention_block: unsupported tensor layout");
        return GEIST_E_UNSUPPORTED;
    }

    const size_t rows = residual_rows;
    if (block->q_position > SIZE_MAX - rows ||
        block->q_position + rows > block->kv_len ||
        residual_cols != d || normed_rows != rows || normed_cols != d ||
        q_rows != rows || q_cols != q_out ||
        k_rows != rows || k_cols != kv_out ||
        v_rows != rows || v_cols != kv_out ||
        attn_rows != rows || attn_cols != q_out ||
        o_rows != rows || o_cols != d ||
        out_rows != rows || out_cols != d ||
        norm_n != d || q_norm_n != block->head_dim ||
        k_norm_n != block->head_dim ||
        v_norm_n != block->head_dim ||
        post_norm_n != d ||
        cos_cols != block->head_dim || sin_cols != block->head_dim ||
        cos_stride != block->head_dim || sin_stride != block->head_dim ||
        cos_rows != sin_rows ||
        !((cos_rows == rows) ||
          (cos_rows >= block->q_position + rows)) ||
        k_cache_len != block->kv_len ||
        v_cache_len != block->kv_len ||
        k_cache_heads != block->kv_heads ||
        v_cache_heads != block->kv_heads ||
        k_cache_hd != block->head_dim ||
        v_cache_hd != block->head_dim ||
        q_w_rows != q_out || q_w_cols != d ||
        k_w_rows != kv_out || k_w_cols != d ||
        v_w_rows != kv_out || v_w_cols != d ||
        o_w_rows != d || o_w_cols != q_out) {
        geist_backend_set_error(
            be, GEIST_E_UNSUPPORTED,
            "metal attention_block: unsupported tensor shape");
        return GEIST_E_UNSUPPORTED;
    }

    if (rows > UINT32_MAX || d > UINT32_MAX || q_out > UINT32_MAX ||
        kv_out > UINT32_MAX || block->kv_len > UINT32_MAX ||
        block->q_position > UINT32_MAX ||
        block->q_heads > UINT32_MAX || block->kv_heads > UINT32_MAX ||
        block->head_dim > UINT32_MAX ||
        residual_offset > UINT32_MAX || normed_offset > UINT32_MAX ||
        q_offset > UINT32_MAX || k_offset > UINT32_MAX ||
        v_offset > UINT32_MAX || attn_offset > UINT32_MAX ||
        o_offset > UINT32_MAX || out_offset > UINT32_MAX ||
        norm_offset > UINT32_MAX || q_norm_offset > UINT32_MAX ||
        k_norm_offset > UINT32_MAX || v_norm_offset > UINT32_MAX ||
        post_norm_offset > UINT32_MAX || cos_offset > UINT32_MAX ||
        sin_offset > UINT32_MAX || k_cache_offset > UINT32_MAX ||
        v_cache_offset > UINT32_MAX || q_w_offset > UINT32_MAX ||
        k_w_offset > UINT32_MAX || v_w_offset > UINT32_MAX ||
        o_w_offset > UINT32_MAX) {
        return GEIST_E_INVALID_ARG;
    }
    ...
    const struct geist_tensor *owned[] = {
        ...
        block->post_attn_norm_weight, block->normed_scratch,
        block->q_scratch, block->k_scratch, block->v_scratch,
        block->attn_scratch, block->o_scratch, block->out,
    };
    for (size_t i = 0; i < sizeof(owned) / sizeof(owned[0]); i++) {
        if (!metal_tensor_owned_by(owned[i], st)) {
            return GEIST_E_INVALID_ARG;
    ...
    s = metal_ensure_attention_pipeline(be);
    if (s != GEIST_OK) {
        return s;
    }
    struct geist_buffer *q_qk_packed = nullptr;
    struct geist_buffer *k_qk_packed = nullptr;
    if (st->use_fused_attention_qk &&
        st->use_fused_attention_qk_nt4 &&
        rows == 1u &&
        st->use_q4k_nt4 && st->use_q4k_n4) {
        q_qk_packed = metal_q4k_nt4_cache_find_tensor(
            st, block->q_proj_weight);
        k_qk_packed = metal_q4k_nt4_cache_find_tensor(
            st, block->k_proj_weight);
    }
    const bool use_fused_attention_qk_nt4 =
        q_qk_packed != nullptr && k_qk_packed != nullptr;
    const bool use_fused_attention_qk =
        st->use_fused_attention_qk && rows > 1u;
    if (use_fused_attention_qk || use_fused_attention_qk_nt4) {
        s = metal_ensure_q4k_qk_pipeline(be);
        if (s != GEIST_OK) {
            return s;
        }
    }
    if (st->use_q4k_w4a8 && rows == 1u) {
        size_t w4a8_n_in = q_out;
        if (!use_fused_attention_qk && !use_fused_attention_qk_nt4 &&
            d > w4a8_n_in) {
            w4a8_n_in = d;
        }
        if (!v_w_q6 && d > w4a8_n_in) {
            w4a8_n_in = d;
        }
        s = metal_ensure_q4k_w4a8_scratch(be, w4a8_n_in);
        if (s != GEIST_OK) {
    ...
    if (st->sequence_active) {
        if (st->sequence_compute_encoder == nullptr) {
            geist_backend_set_error(
                be, GEIST_E_BACKEND,
                "metal attention_block: command sequence has no encoder");
            return GEIST_E_BACKEND;
        }
        enc = st->sequence_compute_encoder;
    } else {
        ...
        if (cmd == nullptr) {
            geist_backend_set_error(be, GEIST_E_BACKEND,
                                    "metal attention_block: command buffer failed");
            return GEIST_E_BACKEND;
        }
        enc = metal_msg_send_id0(st, cmd, "computeCommandEncoder");
        if (enc == nullptr) {
            geist_backend_set_error(be, GEIST_E_BACKEND,
                                    "metal attention_block: encoder failed");
            return GEIST_E_BACKEND;
        }
    }
    ...
    const struct metal_q4k_params q_params = {
        .n_in = (uint32_t) d,
        .n_out = (uint32_t) q_out,
        .rows = (uint32_t) rows,
        .blocks_per_row = (uint32_t) (d / METAL_Q4K_BLOCK_ELEMS),
        .x_offset = (uint32_t) normed_offset,
        .w_byte_offset = (uint32_t) q_w_offset,
        .y_offset = (uint32_t) q_offset,
        .x_row_stride = (uint32_t) normed_stride,
        .y_row_stride = (uint32_t) q_stride,
    };
    const struct metal_q4k_params k_params = {
        .n_in = (uint32_t) d,
        .n_out = (uint32_t) kv_out,
        .rows = (uint32_t) rows,
        .blocks_per_row = (uint32_t) (d / METAL_Q4K_BLOCK_ELEMS),
        .x_offset = (uint32_t) normed_offset,
        .w_byte_offset = (uint32_t) k_w_offset,
        .y_offset = (uint32_t) k_offset,
        .x_row_stride = (uint32_t) normed_stride,
        .y_row_stride = (uint32_t) k_stride,
    };
    const struct metal_q4k_qk_params qk_params = {
        .n_in = (uint32_t) d,
        .q_out = (uint32_t) q_out,
        .k_out = (uint32_t) kv_out,
        .rows = (uint32_t) rows,
        .blocks_per_row = (uint32_t) (d / METAL_Q4K_BLOCK_ELEMS),
        .x_offset = (uint32_t) normed_offset,
        .q_w_byte_offset = (uint32_t) q_w_offset,
        .k_w_byte_offset = (uint32_t) k_w_offset,
        .q_y_offset = (uint32_t) q_offset,
        .k_y_offset = (uint32_t) k_offset,
        .x_row_stride = (uint32_t) normed_stride,
        .q_y_row_stride = (uint32_t) q_stride,
        .k_y_row_stride = (uint32_t) k_stride,
    };
    const struct metal_q4k_params v_params = {
        .n_in = (uint32_t) d,
        .n_out = (uint32_t) kv_out,
        .rows = (uint32_t) rows,
        .blocks_per_row =
            (uint32_t) (d / (v_w_q6 ? METAL_Q6K_BLOCK_ELEMS
                                    : METAL_Q4K_BLOCK_ELEMS)),
        .x_offset = (uint32_t) normed_offset,
        .w_byte_offset = (uint32_t) v_w_offset,
        .y_offset = (uint32_t) v_offset,
        .x_row_stride = (uint32_t) normed_stride,
        .y_row_stride = (uint32_t) v_stride,
    };
    const uint32_t rope_row_offset =
        cos_rows == rows ? 0u : (uint32_t) block->q_position;
    const struct metal_norm_rope_params q_norm_rope_params = {
        .rows = (uint32_t) rows,
        .heads = (uint32_t) block->q_heads,
        .head_dim = (uint32_t) block->head_dim,
        .x_offset = (uint32_t) q_offset,
        .w_offset = (uint32_t) q_norm_offset,
        .cos_offset = (uint32_t) cos_offset,
        .sin_offset = (uint32_t) sin_offset,
        .x_row_stride = (uint32_t) q_stride,
        .rope_row_stride = (uint32_t) cos_stride,
        .rope_row_offset = rope_row_offset,
        .eps = block->eps,
    };
    const struct metal_k_norm_rope_append_params k_norm_rope_append_params = {
        .rows = (uint32_t) rows,
        .heads = (uint32_t) block->kv_heads,
        .head_dim = (uint32_t) block->head_dim,
        .x_offset = (uint32_t) k_offset,
        .w_offset = (uint32_t) k_norm_offset,
        .cos_offset = (uint32_t) cos_offset,
        .sin_offset = (uint32_t) sin_offset,
        .cache_offset = (uint32_t) k_cache_offset,
        .x_row_stride = (uint32_t) k_stride,
        .rope_row_stride = (uint32_t) cos_stride,
        .rope_row_offset = rope_row_offset,
        .q_position = (uint32_t) block->q_position,
        .eps = block->eps,
    };
    const struct metal_v_norm_append_params v_norm_append_params = {
        .rows = (uint32_t) rows,
        .heads = (uint32_t) block->kv_heads,
        .head_dim = (uint32_t) block->head_dim,
        .x_offset = (uint32_t) v_offset,
        .w_offset = (uint32_t) v_norm_offset,
        .cache_offset = (uint32_t) v_cache_offset,
        .x_row_stride = (uint32_t) v_stride,
        .q_position = (uint32_t) block->q_position,
        .eps = block->eps,
    };
    const struct metal_rows_params q_norm_params = {
        .rows = (uint32_t) (rows * block->q_heads),
        .cols = (uint32_t) block->head_dim,
        .x_offset = (uint32_t) q_offset,
        .w_offset = (uint32_t) q_norm_offset,
        .y_offset = (uint32_t) q_offset,
        .x_row_stride = (uint32_t) block->head_dim,
        .y_row_stride = (uint32_t) block->head_dim,
        .eps = block->eps,
    };
    const struct metal_rows_params k_norm_params = {
        .rows = (uint32_t) (rows * block->kv_heads),
        .cols = (uint32_t) block->head_dim,
        .x_offset = (uint32_t) k_offset,
        .w_offset = (uint32_t) k_norm_offset,
        .y_offset = (uint32_t) k_offset,
    ...
    const struct metal_rows_params v_norm_params = {
        .rows = (uint32_t) (rows * block->kv_heads),
        .cols = (uint32_t) block->head_dim,
        .x_offset = (uint32_t) v_offset,
        .w_offset = (uint32_t) v_norm_offset,
    ...
    const struct metal_rope_params q_rope_params = {
        .rows = (uint32_t) rows,
        .heads = (uint32_t) block->q_heads,
        .head_dim = (uint32_t) block->head_dim,
        .x_offset = (uint32_t) q_offset,
    ...
    const struct metal_rope_params k_rope_params = {
        .rows = (uint32_t) rows,
        .heads = (uint32_t) block->kv_heads,
        .head_dim = (uint32_t) block->head_dim,
        .x_offset = (uint32_t) k_offset,
    ...
    const struct metal_kv_append_params append_params = {
        .elems = (uint32_t) (rows * kv_out),
        .kv_out = (uint32_t) kv_out,
        .k_offset = (uint32_t) k_offset,
        .v_offset = (uint32_t) v_offset,
        .k_cache_offset = (uint32_t) k_cache_offset,
        .v_cache_offset = (uint32_t) v_cache_offset,
        .q_position = (uint32_t) block->q_position,
    };
    const struct metal_attention_params attention_params = {
        .rows = (uint32_t) rows,
        .kv_len = (uint32_t) block->kv_len,
        .q_heads = (uint32_t) block->q_heads,
        .kv_heads = (uint32_t) block->kv_heads,
        .head_dim = (uint32_t) block->head_dim,
        .q_position = (uint32_t) block->q_position,
        .sliding_window = (uint32_t) block->sliding_window,
        .q_offset = (uint32_t) q_offset,
        .k_cache_offset = (uint32_t) k_cache_offset,
        .v_cache_offset = (uint32_t) v_cache_offset,
        .y_offset = (uint32_t) attn_offset,
    };
    const struct metal_q4k_params o_params = {
        .n_in = (uint32_t) q_out,
        .n_out = (uint32_t) d,
        .rows = (uint32_t) rows,
        .blocks_per_row = (uint32_t) (q_out / METAL_Q4K_BLOCK_ELEMS),
        .x_offset = (uint32_t) attn_offset,
        .w_byte_offset = (uint32_t) o_w_offset,
        .y_offset = (uint32_t) o_offset,
        .x_row_stride = (uint32_t) attn_stride,
        .y_row_stride = (uint32_t) o_stride,
    };
    const struct metal_post_norm_params post_params = {
        .rows = (uint32_t) rows,
        .cols = (uint32_t) d,
        .residual_offset = (uint32_t) residual_offset,
        .x_offset = (uint32_t) o_offset,
        .w_offset = (uint32_t) post_norm_offset,
        .y_offset = (uint32_t) out_offset,
        .residual_row_stride = (uint32_t) residual_stride,
        .x_row_stride = (uint32_t) o_stride,
        .y_row_stride = (uint32_t) out_stride,
        .eps = block->eps,
    };

    if (st->sequence_active) {
        metal_decode_replay_record_attention(
            st, block, v_w_q6, use_fused_attention_qk,
            use_fused_attention_qk_nt4, cos_rows != rows,
            q_qk_packed, k_qk_packed,
            &norm_params, &q_params, &k_params, &qk_params, &v_params,
            &q_norm_rope_params, &k_norm_rope_append_params,
            &v_norm_append_params, &attention_params, &o_params,
            &post_params);
    }

    metal_encode_rmsnorm_rows(st, enc, block->residual,
                              block->attn_norm_weight,
                              block->normed_scratch, &norm_params);
    if (use_fused_attention_qk_nt4) {
        metal_encode_q4k_qk_nt4(st, enc, block->normed_scratch,
                                q_qk_packed, k_qk_packed,
                                block->q_scratch, block->k_scratch,
                                &qk_params);
    } else if (use_fused_attention_qk) {
        metal_encode_q4k_qk(st, enc, block->normed_scratch,
                            block->q_proj_weight, block->k_proj_weight,
                            block->q_scratch, block->k_scratch,
                            &qk_params);
    } else {
        metal_encode_q4k_linear(st, enc, block->normed_scratch,
                                block->q_proj_weight, block->q_scratch,
                                &q_params, rows > 1);
        metal_encode_q4k_linear(st, enc, block->normed_scratch,
                                block->k_proj_weight, block->k_scratch,
                                &k_params, rows > 1);
    }
    if (v_w_q6) {
        metal_encode_q6k_linear(st, enc, block->normed_scratch,
                                block->v_proj_weight, block->v_scratch,
                                &v_params, rows > 1);
    } else {
        metal_encode_q4k_linear(st, enc, block->normed_scratch,
                                block->v_proj_weight, block->v_scratch,
    ...
    const bool use_fused_attention_norm =
        st->sequence_active || st->use_fused_attention_norm;
    if (use_fused_attention_norm) {
        const bool use_qnorm_attention =
            metal_can_fuse_qnorm_attention(st, block->k_cache,
                                           &q_norm_rope_params,
                                           &attention_params);
        if (!use_qnorm_attention) {
            metal_encode_q_norm_rope_rows(st, enc, block->q_scratch,
                                          block->q_norm_weight, block->cos,
                                          block->sin, &q_norm_rope_params);
        }
        if (metal_can_fuse_kv_norm_append(st, block->k_cache, block->v_cache,
                                          &k_norm_rope_append_params,
                                          &v_norm_append_params)) {
            metal_encode_kv_norm_append_rows(
                st, enc, block->k_scratch, block->v_scratch,
                block->k_norm_weight, block->v_norm_weight, block->cos,
                block->sin, block->k_cache, block->v_cache,
                &k_norm_rope_append_params, &v_norm_append_params);
        } else {
            metal_encode_k_norm_rope_append_rows(
                st, enc, block->k_scratch, block->k_norm_weight,
                block->cos, block->sin, block->k_cache,
                &k_norm_rope_append_params);
            metal_encode_v_norm_append_rows(st, enc, block->v_scratch,
                                            block->v_norm_weight,
                                            block->v_cache,
                                            &v_norm_append_params);
        }
        if (use_qnorm_attention) {
            metal_encode_attention_qnorm_rows(
                st, enc, block->q_scratch, block->q_norm_weight, block->cos,
                block->sin, block->k_cache, block->v_cache,
                block->attn_scratch, &q_norm_rope_params, &attention_params);
        } else {
            metal_encode_attention_rows(st, enc, block->q_scratch,
                                        block->k_cache, block->v_cache,
                                        block->attn_scratch,
                                        &attention_params);
        }
    } else {
        metal_encode_rmsnorm_rows(st, enc, block->q_scratch,
                                  block->q_norm_weight, block->q_scratch,
                                  &q_norm_params);
        metal_encode_rmsnorm_rows(st, enc, block->k_scratch,
                                  block->k_norm_weight, block->k_scratch,
                                  &k_norm_params);
        metal_encode_rmsnorm_rows(st, enc, block->v_scratch,
                                  block->v_norm_weight, block->v_scratch,
                                  &v_norm_params);
        metal_encode_rope_rows(st, enc, block->q_scratch, block->cos,
                               block->sin, &q_rope_params);
        metal_encode_rope_rows(st, enc, block->k_scratch, block->cos,
                               block->sin, &k_rope_params);
        metal_encode_kv_append_rows(st, enc, block->k_scratch,
                                    block->v_scratch, block->k_cache,
                                    block->v_cache, &append_params);
        metal_encode_attention_rows(st, enc, block->q_scratch, block->k_cache,
                                    block->v_cache, block->attn_scratch,
                                    &attention_params);
    }
    metal_encode_q4k_linear(st, enc, block->attn_scratch,
                            block->o_proj_weight, block->o_scratch,
                            &o_params, rows > 1);
    metal_encode_rmsnorm_add_rows(st, enc, block->residual,
                                  block->o_scratch,
                                  block->post_attn_norm_weight, block->out,
                                  &post_params);
    ...
    if (err != nullptr) {
        const char *msg = metal_nserror_message(st, err);
        geist_backend_set_error(be, GEIST_E_BACKEND,
                                "metal attention_block: command failed%s%s",
                                msg != nullptr ? ": " : "",
                                msg != nullptr ? msg : "");
        return GEIST_E_BACKEND;
    }
...
[[nodiscard]] static enum geist_status metal_attention_query_block(
    struct geist_backend *be,
    const struct geist_backend_attention_query_block *block) {
    ...
    if (block->d_model == 0 || block->q_heads == 0 ||
        ...
        block->q_heads % block->kv_heads != 0 ||
        (block->head_dim & 1u) != 0 ||
        block->q_position >= block->kv_len || block->head_dim > 512u ||
        block->post_attn_norm_weight == nullptr ||
        block->post_attn_scratch == nullptr) {
        return GEIST_E_INVALID_ARG;
    }
    ...
    const size_t q_out = block->q_heads * block->head_dim;
    const size_t d = block->d_model;
    size_t residual_rows = 0, residual_cols = 0, residual_offset = 0;
    size_t residual_stride = 0;
    size_t normed_rows = 0, normed_cols = 0, normed_offset = 0;
    size_t normed_stride = 0;
    size_t q_rows = 0, q_cols = 0, q_offset = 0, q_stride = 0;
    size_t attn_rows = 0, attn_cols = 0, attn_offset = 0;
    size_t attn_stride = 0;
    size_t o_rows = 0, o_cols = 0, o_offset = 0, o_stride = 0;
    size_t out_rows = 0, out_cols = 0, out_offset = 0, out_stride = 0;
    size_t norm_n = 0, norm_offset = 0;
    size_t q_norm_n = 0, q_norm_offset = 0;
    size_t post_norm_n = 0, post_norm_offset = 0;
    size_t cos_rows = 0, cos_cols = 0, cos_offset = 0, cos_stride = 0;
    size_t sin_rows = 0, sin_cols = 0, sin_offset = 0, sin_stride = 0;
    size_t k_cache_len = 0, k_cache_heads = 0, k_cache_hd = 0;
    size_t k_cache_offset = 0;
    size_t v_cache_len = 0, v_cache_heads = 0, v_cache_hd = 0;
    size_t v_cache_offset = 0;
    size_t q_w_rows = 0, q_w_cols = 0, q_w_offset = 0;
    size_t o_w_rows = 0, o_w_cols = 0, o_w_offset = 0;
    const bool kv_cache_f32 =
        metal_tensor_is_f32_3d(block->k_cache, &k_cache_len,
                               &k_cache_heads, &k_cache_hd,
                               &k_cache_offset) &&
    ...
    if (!metal_tensor_is_f32_matrix(block->residual, &residual_rows,
        ...
        geist_backend_set_error(
            be, GEIST_E_UNSUPPORTED,
            "metal attention_query_block: unsupported tensor layout");
        return GEIST_E_UNSUPPORTED;
    }
    ...
    if (block->q_position > SIZE_MAX - rows ||
        ...
        cos_stride != block->head_dim || sin_stride != block->head_dim ||
        cos_rows != sin_rows ||
        !((cos_rows == rows) || (cos_rows >= block->q_position + rows)) ||
        k_cache_len != block->kv_len ||
        v_cache_len != block->kv_len ||
        k_cache_heads != block->kv_heads ||
        v_cache_heads != block->kv_heads ||
        ...
        o_w_rows != d || o_w_cols != q_out) {
        geist_backend_set_error(be, GEIST_E_UNSUPPORTED,
                                "metal attention_query_block: shape mismatch");
        return GEIST_E_UNSUPPORTED;
    }
    if (rows > UINT32_MAX || d > UINT32_MAX || q_out > UINT32_MAX ||
        block->q_heads > UINT32_MAX || block->kv_heads > UINT32_MAX ||
        ...
        block->q_position > UINT32_MAX ||
        residual_offset > UINT32_MAX || normed_offset > UINT32_MAX ||
        q_offset > UINT32_MAX || attn_offset > UINT32_MAX ||
        o_offset > UINT32_MAX || out_offset > UINT32_MAX ||
        norm_offset > UINT32_MAX || q_norm_offset > UINT32_MAX ||
        post_norm_offset > UINT32_MAX || cos_offset > UINT32_MAX ||
        sin_offset > UINT32_MAX || k_cache_offset > UINT32_MAX ||
        v_cache_offset > UINT32_MAX || q_w_offset > UINT32_MAX ||
        o_w_offset > UINT32_MAX || residual_stride > UINT32_MAX ||
        normed_stride > UINT32_MAX || q_stride > UINT32_MAX ||
        attn_stride > UINT32_MAX || o_stride > UINT32_MAX ||
        out_stride > UINT32_MAX || cos_stride > UINT32_MAX) {
        return GEIST_E_INVALID_ARG;
    }
    ...
    const struct geist_tensor *owned[] = {
        ...
        block->o_scratch, block->post_attn_scratch, block->out,
    };
    for (size_t i = 0; i < sizeof(owned) / sizeof(owned[0]); i++) {
        if (!metal_tensor_owned_by(owned[i], st)) {
            return GEIST_E_INVALID_ARG;
    ...
    enum geist_status s = metal_ensure_q4k_pipeline(be);
    if (s == GEIST_OK) { s = metal_ensure_attention_pipeline(be); }
    if (s != GEIST_OK) { return s; }
    if (st->use_q4k_w4a8 && rows == 1u) {
        const size_t w4a8_n_in = d > q_out ? d : q_out;
        s = metal_ensure_q4k_w4a8_scratch(be, w4a8_n_in);
        if (s != GEIST_OK) {
            return s;
        }
    ...
    if (st->sequence_active) {
        if (st->sequence_compute_encoder == nullptr) {
            geist_backend_set_error(
                be, GEIST_E_BACKEND,
                "metal attention_query_block: command sequence has no encoder");
            return GEIST_E_BACKEND;
        }
        metal_decode_replay_record_attention_query(st, block);
        enc = st->sequence_compute_encoder;
    } else {
        cmd = metal_msg_send_id0(st, st->command_queue, "commandBuffer");
        enc = cmd != nullptr
        ...
        if (cmd == nullptr || enc == nullptr) {
            geist_backend_set_error(
                be, GEIST_E_BACKEND,
                "metal attention_query_block: command encoder failed");
            return GEIST_E_BACKEND;
        }
    }

    const uint32_t rope_row_offset = cos_rows == rows
                                         ? 0u
    ...
    metal_encode_q4k_linear(st, enc, block->normed_scratch,
                            block->q_proj_weight, block->q_scratch,
                            &q_params, rows > 1);
    if (metal_can_fuse_qnorm_attention(st, block->k_cache,
                                       &q_norm_rope_params,
                                       &attention_params)) {
        metal_encode_attention_qnorm_rows(
            st, enc, block->q_scratch, block->q_norm_weight, block->cos,
            block->sin, block->k_cache, block->v_cache, block->attn_scratch,
            &q_norm_rope_params, &attention_params);
    } else {
        metal_encode_q_norm_rope_rows(st, enc, block->q_scratch,
                                      block->q_norm_weight, block->cos,
                                      block->sin, &q_norm_rope_params);
    ...
    if (err != nullptr) {
        ...
        geist_backend_set_error(
            be, GEIST_E_BACKEND,
            "metal attention_query_block: command failed%s%s",
...
[[nodiscard]] static enum geist_status metal_command_sequence_read_token(
    struct geist_backend *be,
    geist_token_t *out_token) {
    ...
    *out_token = -1;
    ...
    struct metal_state *st = be->state;
    if (st->sequence_active || !st->captured_greedy_token_pending ||
        st->argmax_result_mapped == nullptr ||
        st->captured_greedy_vocab_size == 0) {
    ...
    if (token_u32 >= st->captured_greedy_vocab_size ||
        ...
        geist_backend_set_error(
            be, GEIST_E_BACKEND,
            "metal command_sequence_read_token: invalid token id %u",
            token_u32);
        return GEIST_E_BACKEND;
    }
    st->captured_greedy_token_pending = false;
...
[[nodiscard]] static enum geist_status metal_command_sequence_read_tokens(
    struct geist_backend *be,
    size_t n,
    geist_token_t out_tokens[static n]) {

    if (n == 0 || out_tokens == nullptr) {
        return GEIST_E_INVALID_ARG;
    }
    for (size_t i = 0; i < n; i++) {
        out_tokens[i] = -1;
    }
    if (be == nullptr || be->state == nullptr || n > UINT32_MAX) {
        return GEIST_E_INVALID_ARG;
    }
    struct metal_state *st = be->state;
    if (st->sequence_active || !st->captured_greedy_token_pending ||
    ...
    for (size_t row = 0; row < n; row++) {
        ...
        if (token_u32 >= st->captured_greedy_vocab_size ||
            ...
            geist_backend_set_error(
                be, GEIST_E_BACKEND,
                "metal command_sequence_read_tokens: invalid token id %u",
                token_u32);
            return GEIST_E_BACKEND;
        }
        out_tokens[row] = (geist_token_t) token_u32;
...
[[nodiscard]] static enum geist_status metal_decode_replay_run_op(
    struct geist_backend *be,
    const struct metal_decode_replay_op *op,
    geist_token_t token_id,
    size_t q_position) {

    if (be == nullptr || be->state == nullptr || op == nullptr) {
        return GEIST_E_INVALID_ARG;
    }
    struct metal_state *st = be->state;
    void *enc = st->sequence_compute_encoder;
    if (enc == nullptr) {
        geist_backend_set_error(be, GEIST_E_BACKEND,
                                "metal decode replay: command sequence has no encoder");
        return GEIST_E_BACKEND;
    }
    switch (op->kind) {
    case METAL_DECODE_REPLAY_EMBED_LOOKUP_SCALED: {
        struct metal_decode_replay_embed r = op->u.embed;
        if (token_id < 0 || (uint64_t) token_id > UINT32_MAX) {
            return GEIST_E_INVALID_ARG;
        }
        const struct metal_embed_params params = {
            .n = r.params.n,
            .dtype = r.params.dtype,
            .blocks_per_row = r.params.blocks_per_row,
            .w_byte_offset = r.params.w_byte_offset,
            .y_offset = r.params.y_offset,
            .token_id = (uint32_t) token_id,
            .scale = r.params.scale,
        };
        metal_encode_embed_lookup_scaled(st, enc, &r.table, &r.out,
                                         &params);
        st->sequence_has_work = true;
        return GEIST_OK;
    }
    case METAL_DECODE_REPLAY_F32_LINEAR: {
        struct metal_decode_replay_f32_linear r = op->u.f32_linear;
        const struct metal_f32_params params = {
            .n_in = r.params.n_in,
            .n_out = r.params.n_out,
            .rows = r.params.rows,
            .x_offset = r.params.x_offset,
            .w_offset = r.params.w_offset,
            .y_offset = r.params.y_offset,
            .x_row_stride = r.params.x_row_stride,
            .y_row_stride = r.params.y_row_stride,
        };
        metal_encode_f32_matmul(st, enc, &r.x, &r.w, &r.y, &params);
        st->sequence_has_work = true;
        return GEIST_OK;
    }
    case METAL_DECODE_REPLAY_RMSNORM: {
        struct metal_decode_replay_rmsnorm r = op->u.rmsnorm;
        const struct metal_rows_params params = {
            .rows = r.params.rows,
            .cols = r.params.cols,
            .x_offset = r.params.x_offset,
        ...
        metal_encode_rmsnorm_rows(st, enc, &r.x, &r.w, &r.y, &params);
        st->sequence_has_work = true;
        return GEIST_OK;
    }
    case METAL_DECODE_REPLAY_ADD: {
        struct metal_decode_replay_binary r = op->u.binary;
        const struct metal_binary_rows_params params = {
            .rows = r.params.rows,
            .cols = r.params.cols,
            .a_offset = r.params.a_offset,
            .b_offset = r.params.b_offset,
            .y_offset = r.params.y_offset,
            .a_row_stride = r.params.a_row_stride,
            .b_row_stride = r.params.b_row_stride,
            .y_row_stride = r.params.y_row_stride,
        };
        metal_encode_add_rows(st, enc, &r.a, &r.b, &r.y, &params);
        st->sequence_has_work = true;
        return GEIST_OK;
    }
    case METAL_DECODE_REPLAY_SCALE_F32: {
        struct metal_decode_replay_scale r = op->u.scale;
        const struct metal_scale_rows_params params = {
            .rows = r.params.rows,
            .cols = r.params.cols,
            .x_offset = r.params.x_offset,
        ...
        metal_encode_scale_rows(st, enc, &r.x, &r.y, &params);
        st->sequence_has_work = true;
        return GEIST_OK;
    }
    case METAL_DECODE_REPLAY_ATTENTION: {
        struct metal_decode_replay_attention r = op->u.attention;
        const uint32_t rows = r.attention_params.rows;
        if (rows == 0 || q_position > UINT32_MAX ||
            q_position > (size_t) UINT32_MAX - rows) {
            return GEIST_E_INVALID_ARG;
        }
        const uint32_t qpos = (uint32_t) q_position;
        const uint32_t kv_len = qpos + rows;
        const uint32_t rope_row_offset =
            r.rope_uses_q_position ? qpos
                                    : r.q_norm_rope_params.rope_row_offset;
        const struct metal_rows_params norm_params = {
            .rows = r.norm_params.rows,
            .cols = r.norm_params.cols,
            .x_offset = r.norm_params.x_offset,
            .w_offset = r.norm_params.w_offset,
            .y_offset = r.norm_params.y_offset,
            .x_row_stride = r.norm_params.x_row_stride,
            .y_row_stride = r.norm_params.y_row_stride,
            .eps = r.norm_params.eps,
        };
        const struct metal_q4k_params q_params = {
            .n_in = r.q_params.n_in,
            .n_out = r.q_params.n_out,
            .rows = r.q_params.rows,
            .blocks_per_row = r.q_params.blocks_per_row,
            .x_offset = r.q_params.x_offset,
            .w_byte_offset = r.q_params.w_byte_offset,
            .y_offset = r.q_params.y_offset,
            .x_row_stride = r.q_params.x_row_stride,
            .y_row_stride = r.q_params.y_row_stride,
        };
        const struct metal_q4k_params k_params = {
            .n_in = r.k_params.n_in,
            .n_out = r.k_params.n_out,
            .rows = r.k_params.rows,
            .blocks_per_row = r.k_params.blocks_per_row,
            .x_offset = r.k_params.x_offset,
            .w_byte_offset = r.k_params.w_byte_offset,
            .y_offset = r.k_params.y_offset,
            .x_row_stride = r.k_params.x_row_stride,
            .y_row_stride = r.k_params.y_row_stride,
        };
        const struct metal_q4k_qk_params qk_params = {
            .n_in = r.qk_params.n_in,
            .q_out = r.qk_params.q_out,
            .k_out = r.qk_params.k_out,
            .rows = r.qk_params.rows,
            .blocks_per_row = r.qk_params.blocks_per_row,
            .x_offset = r.qk_params.x_offset,
            .q_w_byte_offset = r.qk_params.q_w_byte_offset,
            .k_w_byte_offset = r.qk_params.k_w_byte_offset,
            .q_y_offset = r.qk_params.q_y_offset,
            .k_y_offset = r.qk_params.k_y_offset,
            .x_row_stride = r.qk_params.x_row_stride,
            .q_y_row_stride = r.qk_params.q_y_row_stride,
            .k_y_row_stride = r.qk_params.k_y_row_stride,
        };
        const struct metal_q4k_params v_params = {
            .n_in = r.v_params.n_in,
            .n_out = r.v_params.n_out,
            .rows = r.v_params.rows,
            .blocks_per_row = r.v_params.blocks_per_row,
            .x_offset = r.v_params.x_offset,
            .w_byte_offset = r.v_params.w_byte_offset,
            .y_offset = r.v_params.y_offset,
            .x_row_stride = r.v_params.x_row_stride,
            .y_row_stride = r.v_params.y_row_stride,
        };
        const struct metal_norm_rope_params q_norm_rope_params = {
            .rows = r.q_norm_rope_params.rows,
            .heads = r.q_norm_rope_params.heads,
            .head_dim = r.q_norm_rope_params.head_dim,
            .x_offset = r.q_norm_rope_params.x_offset,
            .w_offset = r.q_norm_rope_params.w_offset,
            .cos_offset = r.q_norm_rope_params.cos_offset,
            .sin_offset = r.q_norm_rope_params.sin_offset,
            .x_row_stride = r.q_norm_rope_params.x_row_stride,
            .rope_row_stride = r.q_norm_rope_params.rope_row_stride,
            .rope_row_offset = rope_row_offset,
            .eps = r.q_norm_rope_params.eps,
        };
        const struct metal_k_norm_rope_append_params
            k_norm_rope_append_params = {
                .rows = r.k_norm_rope_append_params.rows,
                .heads = r.k_norm_rope_append_params.heads,
                .head_dim = r.k_norm_rope_append_params.head_dim,
                .x_offset = r.k_norm_rope_append_params.x_offset,
                .w_offset = r.k_norm_rope_append_params.w_offset,
                .cos_offset = r.k_norm_rope_append_params.cos_offset,
                .sin_offset = r.k_norm_rope_append_params.sin_offset,
                .cache_offset = r.k_norm_rope_append_params.cache_offset,
                .x_row_stride = r.k_norm_rope_append_params.x_row_stride,
                .rope_row_stride =
                    r.k_norm_rope_append_params.rope_row_stride,
                .rope_row_offset = rope_row_offset,
                .q_position = qpos,
                .eps = r.k_norm_rope_append_params.eps,
            };
        const struct metal_v_norm_append_params v_norm_append_params = {
            .rows = r.v_norm_append_params.rows,
            .heads = r.v_norm_append_params.heads,
            .head_dim = r.v_norm_append_params.head_dim,
            .x_offset = r.v_norm_append_params.x_offset,
            .w_offset = r.v_norm_append_params.w_offset,
            .cache_offset = r.v_norm_append_params.cache_offset,
            .x_row_stride = r.v_norm_append_params.x_row_stride,
            .q_position = qpos,
            .eps = r.v_norm_append_params.eps,
        };
        const struct metal_attention_params attention_params = {
            .rows = r.attention_params.rows,
            .kv_len = kv_len,
            .q_heads = r.attention_params.q_heads,
            .kv_heads = r.attention_params.kv_heads,
            .head_dim = r.attention_params.head_dim,
            .q_position = qpos,
            .sliding_window = r.attention_params.sliding_window,
            .q_offset = r.attention_params.q_offset,
            .k_cache_offset = r.attention_params.k_cache_offset,
            .v_cache_offset = r.attention_params.v_cache_offset,
            .y_offset = r.attention_params.y_offset,
        };
        const struct metal_q4k_params o_params = {
            .n_in = r.o_params.n_in,
            .n_out = r.o_params.n_out,
            .rows = r.o_params.rows,
            .blocks_per_row = r.o_params.blocks_per_row,
            .x_offset = r.o_params.x_offset,
            .w_byte_offset = r.o_params.w_byte_offset,
            .y_offset = r.o_params.y_offset,
            .x_row_stride = r.o_params.x_row_stride,
            .y_row_stride = r.o_params.y_row_stride,
        };
        const struct metal_post_norm_params post_params = {
            .rows = r.post_params.rows,
            .cols = r.post_params.cols,
            .residual_offset = r.post_params.residual_offset,
            .x_offset = r.post_params.x_offset,
            .w_offset = r.post_params.w_offset,
            .y_offset = r.post_params.y_offset,
            .residual_row_stride = r.post_params.residual_row_stride,
            .x_row_stride = r.post_params.x_row_stride,
            .y_row_stride = r.post_params.y_row_stride,
            .eps = r.post_params.eps,
        };

        metal_encode_rmsnorm_rows(st, enc, &r.residual,
                                  &r.attn_norm_weight,
                                  &r.normed_scratch, &norm_params);
        if (r.use_fused_qk_nt4) {
            if (r.q_qk_packed == nullptr || r.k_qk_packed == nullptr) {
                return GEIST_E_INVALID_STATE;
            }
            metal_encode_q4k_qk_nt4(st, enc, &r.normed_scratch,
                                    r.q_qk_packed, r.k_qk_packed,
                                    &r.q_scratch, &r.k_scratch,
                                    &qk_params);
        } else if (r.use_fused_qk) {
            metal_encode_q4k_qk(st, enc, &r.normed_scratch,
                                &r.q_proj_weight, &r.k_proj_weight,
                                &r.q_scratch, &r.k_scratch,
                                &qk_params);
        } else {
            metal_encode_q4k_linear(st, enc, &r.normed_scratch,
                                    &r.q_proj_weight, &r.q_scratch,
                                    &q_params, false);
            metal_encode_q4k_linear(st, enc, &r.normed_scratch,
                                    &r.k_proj_weight, &r.k_scratch,
                                    &k_params, false);
        }
        if (r.v_w_q6) {
            metal_encode_q6k_linear(st, enc, &r.normed_scratch,
                                    &r.v_proj_weight, &r.v_scratch,
                                    &v_params, false);
        } else {
            metal_encode_q4k_linear(st, enc, &r.normed_scratch,
                                    &r.v_proj_weight, &r.v_scratch,
        ...
        const bool use_qnorm_attention =
            metal_can_fuse_qnorm_attention(st, &r.k_cache,
                                           &q_norm_rope_params,
                                           &attention_params);
        if (!use_qnorm_attention) {
            metal_encode_q_norm_rope_rows(st, enc, &r.q_scratch,
                                          &r.q_norm_weight, &r.cos, &r.sin,
                                          &q_norm_rope_params);
        }
        if (metal_can_fuse_kv_norm_append(st, &r.k_cache, &r.v_cache,
                                          &k_norm_rope_append_params,
                                          &v_norm_append_params)) {
            metal_encode_kv_norm_append_rows(
                st, enc, &r.k_scratch, &r.v_scratch, &r.k_norm_weight,
                &r.v_norm_weight, &r.cos, &r.sin, &r.k_cache, &r.v_cache,
                &k_norm_rope_append_params, &v_norm_append_params);
        } else {
            metal_encode_k_norm_rope_append_rows(
                st, enc, &r.k_scratch, &r.k_norm_weight, &r.cos, &r.sin,
                &r.k_cache, &k_norm_rope_append_params);
            metal_encode_v_norm_append_rows(st, enc, &r.v_scratch,
                                            &r.v_norm_weight, &r.v_cache,
                                            &v_norm_append_params);
        }
        if (use_qnorm_attention) {
            metal_encode_attention_qnorm_rows(
                st, enc, &r.q_scratch, &r.q_norm_weight, &r.cos, &r.sin,
                &r.k_cache, &r.v_cache, &r.attn_scratch,
                &q_norm_rope_params, &attention_params);
        } else {
            metal_encode_attention_rows(st, enc, &r.q_scratch, &r.k_cache,
                                        &r.v_cache, &r.attn_scratch,
                                        &attention_params);
        }
        metal_encode_q4k_linear(st, enc, &r.attn_scratch,
                                &r.o_proj_weight, &r.o_scratch,
                                &o_params, false);
        metal_encode_rmsnorm_add_rows(st, enc, &r.residual, &r.o_scratch,
                                      &r.post_attn_norm_weight, &r.out,
                                      &post_params);
        st->sequence_has_work = true;
        return GEIST_OK;
    }
    case METAL_DECODE_REPLAY_ATTENTION_QUERY: {
        struct metal_decode_replay_attention_query r = op->u.attention_query;
        metal_decode_replay_bind_attention_query(&r);
        const size_t rows = r.block.kv_len - r.block.q_position;
        if (rows == 0 || q_position > SIZE_MAX - rows ||
            q_position + rows > (size_t) INT64_MAX) {
            return GEIST_E_INVALID_ARG;
        }
        r.block.q_position = q_position;
        r.block.kv_len = q_position + rows;
        r.k_cache.shape[0] = (int64_t) r.block.kv_len;
        r.v_cache.shape[0] = (int64_t) r.block.kv_len;
        return metal_attention_query_block(be, &r.block);
    }
    case METAL_DECODE_REPLAY_FFN_GEGLU: {
        struct metal_decode_replay_ffn r = op->u.ffn;
        const struct metal_rows_params pre_norm_params = {
            .rows = r.pre_norm_params.rows,
            .cols = r.pre_norm_params.cols,
            .x_offset = r.pre_norm_params.x_offset,
            .w_offset = r.pre_norm_params.w_offset,
            .y_offset = r.pre_norm_params.y_offset,
            .x_row_stride = r.pre_norm_params.x_row_stride,
            .y_row_stride = r.pre_norm_params.y_row_stride,
            .eps = r.pre_norm_params.eps,
        };
        const struct metal_q4k_gate_up_params gate_up_params = {
            .n_in = r.gate_up_params.n_in,
            .n_out = r.gate_up_params.n_out,
            .rows = r.gate_up_params.rows,
            .blocks_per_row = r.gate_up_params.blocks_per_row,
            .x_offset = r.gate_up_params.x_offset,
            .gate_w_byte_offset = r.gate_up_params.gate_w_byte_offset,
            .up_w_byte_offset = r.gate_up_params.up_w_byte_offset,
            .gate_y_offset = r.gate_up_params.gate_y_offset,
            .up_y_offset = r.gate_up_params.up_y_offset,
            .x_row_stride = r.gate_up_params.x_row_stride,
            .y_row_stride = r.gate_up_params.y_row_stride,
        };
        const struct metal_q4k_params down_params = {
            .n_in = r.down_params.n_in,
            .n_out = r.down_params.n_out,
            .rows = r.down_params.rows,
            .blocks_per_row = r.down_params.blocks_per_row,
            .x_offset = r.down_params.x_offset,
            .w_byte_offset = r.down_params.w_byte_offset,
            .y_offset = r.down_params.y_offset,
            .x_row_stride = r.down_params.x_row_stride,
            .y_row_stride = r.down_params.y_row_stride,
        };
        const struct metal_post_norm_params post_params = {
            .rows = r.post_params.rows,
            .cols = r.post_params.cols,
        ...
        metal_encode_rmsnorm_rows(st, enc, &r.residual,
                                  ...
                                  &r.pre_ff_scratch,
                                  &pre_norm_params);
        metal_encode_q4k_gate_up(st, enc, &r.pre_ff_scratch,
                                 &r.gate_weight, &r.up_weight,
                                 &r.gate_scratch, &r.up_scratch,
                                 &gate_up_params);
        if (r.down_w_q6) {
            metal_encode_q6k_linear(st, enc, &r.gate_scratch,
                                    &r.down_weight, &r.ffn_out_scratch,
                                    &down_params,
                                    r.down_params.rows > 1u);
        } else {
            metal_encode_q4k_linear(st, enc, &r.gate_scratch,
                                    &r.down_weight, &r.ffn_out_scratch,
                                    &down_params,
                                    r.down_params.rows > 1u);
        }
        metal_encode_rmsnorm_add_rows(st, enc, &r.residual,
                                      &r.ffn_out_scratch,
                                      &r.post_ffw_norm_weight, &r.out,
                                      &post_params);
        st->sequence_has_work = true;
        return GEIST_OK;
    }
    case METAL_DECODE_REPLAY_PLE: {
        struct metal_decode_replay_ple r = op->u.ple;
        const struct metal_q4k_params gate_q4_params = {
            .n_in = r.gate_q4_params.n_in,
            .n_out = r.gate_q4_params.n_out,
            .rows = r.gate_q4_params.rows,
            .blocks_per_row = r.gate_q4_params.blocks_per_row,
            .x_offset = r.gate_q4_params.x_offset,
            .w_byte_offset = r.gate_q4_params.w_byte_offset,
            .y_offset = r.gate_q4_params.y_offset,
            .x_row_stride = r.gate_q4_params.x_row_stride,
            .y_row_stride = r.gate_q4_params.y_row_stride,
        };
        const struct metal_q4k_params proj_q4_params = {
            .n_in = r.proj_q4_params.n_in,
            .n_out = r.proj_q4_params.n_out,
            .rows = r.proj_q4_params.rows,
            .blocks_per_row = r.proj_q4_params.blocks_per_row,
            .x_offset = r.proj_q4_params.x_offset,
            .w_byte_offset = r.proj_q4_params.w_byte_offset,
            .y_offset = r.proj_q4_params.y_offset,
            .x_row_stride = r.proj_q4_params.x_row_stride,
            .y_row_stride = r.proj_q4_params.y_row_stride,
        };
        const struct metal_f32_params gate_f32_params = {
            .n_in = r.gate_f32_params.n_in,
            .n_out = r.gate_f32_params.n_out,
            .rows = r.gate_f32_params.rows,
            .x_offset = r.gate_f32_params.x_offset,
            .w_offset = r.gate_f32_params.w_offset,
            .y_offset = r.gate_f32_params.y_offset,
            .x_row_stride = r.gate_f32_params.x_row_stride,
            .y_row_stride = r.gate_f32_params.y_row_stride,
        };
        const struct metal_f32_params proj_f32_params = {
            .n_in = r.proj_f32_params.n_in,
            .n_out = r.proj_f32_params.n_out,
            .rows = r.proj_f32_params.rows,
            .x_offset = r.proj_f32_params.x_offset,
            .w_offset = r.proj_f32_params.w_offset,
            .y_offset = r.proj_f32_params.y_offset,
            .x_row_stride = r.proj_f32_params.x_row_stride,
            .y_row_stride = r.proj_f32_params.y_row_stride,
        };
        const struct metal_binary_rows_params act_params = {
            .rows = r.act_params.rows,
            .cols = r.act_params.cols,
            .a_offset = r.act_params.a_offset,
            .b_offset = r.act_params.b_offset,
            .y_offset = r.act_params.y_offset,
            .a_row_stride = r.act_params.a_row_stride,
            .b_row_stride = r.act_params.b_row_stride,
            .y_row_stride = r.act_params.y_row_stride,
        };
        const struct metal_post_norm_params post_params = {
            .rows = r.post_params.rows,
            .cols = r.post_params.cols,
        ...
        bool fused_gate =
            r.gate_w_q4 &&
            metal_try_encode_q4k_ple_gate_nt8(
                st, enc, &r.hidden, &r.per_layer_gate_weight,
                &r.per_layer_input, &r.gate_scratch, &gate_q4_params,
                &act_params);
        if (!fused_gate && r.gate_w_q4) {
            metal_encode_q4k_linear(st, enc, &r.hidden,
                                    &r.per_layer_gate_weight,
                                    &r.gate_scratch, &gate_q4_params,
                                    r.gate_q4_params.rows > 1u);
        } else if (!fused_gate && st->f32_ple_gate_pipeline != nullptr) {
            metal_encode_f32_ple_gate(st, enc, &r.hidden,
                                      &r.per_layer_gate_weight,
                                      &r.per_layer_input,
                                      &r.gate_scratch, &gate_f32_params,
                                      &act_params);
            fused_gate = true;
        } else if (!fused_gate) {
            metal_encode_f32_matmul(st, enc, &r.hidden,
                                    &r.per_layer_gate_weight,
                                    &r.gate_scratch, &gate_f32_params);
        }
        if (!fused_gate) {
            metal_encode_gelu_mul_rows(st, enc, &r.gate_scratch,
                                       &r.per_layer_input, &r.gate_scratch,
                                       &act_params);
        }
        if (!r.proj_w_q4 &&
            metal_can_encode_f32_ple_proj_norm(
                st, &r.hidden, &r.out, &proj_f32_params, &post_params)) {
            metal_encode_f32_ple_proj_norm(
                st, enc, &r.gate_scratch, &r.per_layer_proj_weight,
                &r.hidden, &r.post_per_layer_norm_weight, &r.out,
                &proj_f32_params, &post_params);
        } else if (r.proj_w_q4) {
            metal_encode_q4k_linear(st, enc, &r.gate_scratch,
                                    &r.per_layer_proj_weight,
                                    &r.proj_scratch, &proj_q4_params,
                                    r.proj_q4_params.rows > 1u);
            metal_encode_rmsnorm_add_rows(st, enc, &r.hidden,
                                          &r.proj_scratch,
                                          &r.post_per_layer_norm_weight,
                                          &r.out, &post_params);
        } else {
            metal_encode_f32_matmul(st, enc, &r.gate_scratch,
                                    &r.per_layer_proj_weight,
                                    &r.proj_scratch, &proj_f32_params);
            metal_encode_rmsnorm_add_rows(st, enc, &r.hidden,
                                          &r.proj_scratch,
                                          &r.post_per_layer_norm_weight,
                                          &r.out, &post_params);
    ...
    case METAL_DECODE_REPLAY_GREEDY_HEAD: {
        struct metal_decode_replay_greedy_head r = op->u.greedy_head;
        if (st->argmax_result_mapped == nullptr ||
            r.argmax_params.n == 0) {
            return GEIST_E_INVALID_STATE;
        ...
        const struct metal_f32_params f32_params = {
            .n_in = r.f32_params.n_in,
            .n_out = r.f32_params.n_out,
            .rows = r.f32_params.rows,
            .x_offset = r.f32_params.x_offset,
            .w_offset = r.f32_params.w_offset,
            .y_offset = r.f32_params.y_offset,
            .x_row_stride = r.f32_params.x_row_stride,
            .y_row_stride = r.f32_params.y_row_stride,
        };
        const struct metal_q4k_params q_params = {
            .n_in = r.q_params.n_in,
            .n_out = r.q_params.n_out,
        ...
        const struct metal_argmax_params argmax_params = {
            .n = r.argmax_params.n,
            .x_offset = r.argmax_params.x_offset,
        };

        uint32_t sentinel = UINT32_MAX;
        memcpy(st->argmax_result_mapped, &sentinel, sizeof(sentinel));

        metal_encode_rmsnorm_rows(st, enc, &r.hidden, &r.norm_weight,
                                  &r.normed_scratch, &norm_params);
        if (r.lm_head_f32) {
            metal_encode_f32_matmul(st, enc, &r.normed_scratch,
                                    &r.lm_head_weight, &r.logits,
                                    &f32_params);
        } else if (r.lm_head_q4) {
            metal_encode_q4k_linear(st, enc, &r.normed_scratch,
                                    &r.lm_head_weight, &r.logits,
                                    &q_params, false);
        } else if (r.lm_head_q6) {
            metal_encode_q6k_linear(st, enc, &r.normed_scratch,
                                    &r.lm_head_weight, &r.logits,
                                    &q_params, false);
        } else {
        ...
        metal_encode_argmax_f32(st, enc, &r.logits, &argmax_params);
        st->sequence_has_work = true;
        st->captured_greedy_token_pending = true;
        st->captured_greedy_vocab_size = r.argmax_params.n;
        st->captured_greedy_token_count = 1u;
        return GEIST_OK;
    }
    default:
...
metal_command_sequence_replay_decode_greedy_step(
    ...
    *out_token = -1;
    ...
    st->decode_replay_replaying = true;
    int sequence_token = 0;
    enum geist_status s = metal_command_sequence_begin(
        be, GEIST_COMMAND_SEQUENCE_DECODE_GREEDY_STEP, &sequence_token);
    if (s != GEIST_OK) {
        st->decode_replay_replaying = false;
        return s;
    }

    const size_t op_count = st->decode_replay_op_count;
    for (size_t i = 0; s == GEIST_OK && i < op_count; i++) {
        s = metal_decode_replay_run_op(be, &st->decode_replay_ops[i],
                                       token_id, q_position);
    }

    enum geist_status end_s =
        metal_command_sequence_end(be, sequence_token, s == GEIST_OK);
    st->decode_replay_replaying = false;
    if (s != GEIST_OK) {
        return s;
    }
    ...
    return metal_command_sequence_read_token(be, out_token);
}
...
[[nodiscard]] static enum geist_status metal_prepare_weight_layout(
    struct geist_backend *be,
    const struct geist_tensor *w) {
    ...
    if (w->dtype == GEIST_DTYPE_Q4_K) {
        if (!st->use_q4k_nt4 ||
            n_out > st->q4k_nt4_max_n_out ||
            (n_in % Q4_K_BLOCK_ELEMS) != 0) {
            return GEIST_OK;
        }
        if (metal_q4k_nt4_cache_find(st, w, n_in, n_out) != nullptr) {
            return GEIST_OK;
        }

        struct geist_buffer *packed = nullptr;
        return metal_q4k_nt4_cache_get_from_device(be, w, n_in, n_out,
                                                   &packed);
    }
    if (!st->use_q6k_nt4 ||
        n_out < METAL_Q6K_NT4_MIN_N_OUT ||
        n_out > METAL_Q6K_NT4_MAX_N_OUT ||
        (n_in % METAL_Q6K_BLOCK_ELEMS) != 0) {
        return GEIST_OK;
    }
    struct geist_buffer *packed = nullptr;
    if (metal_q6k_nt4_cache_find(st, w, n_in, n_out) != nullptr) {
        return GEIST_OK;
    }
    return metal_q6k_nt4_cache_get_from_device(be, w, n_in, n_out, &packed);
}

[[nodiscard]] static enum geist_status metal_prepare_weight_layout_from_host(
    struct geist_backend *be,
    const struct geist_tensor *w,
    size_t raw_bytes,
    const uint8_t raw[static raw_bytes]) {
    ...
    if (w->dtype == GEIST_DTYPE_Q4_K) {
        ...
        struct geist_buffer *packed = nullptr;
        return metal_q4k_nt4_cache_get(be, w, n_in, n_out, raw_bytes, raw,
                                       &packed);
    }
    if (!st->use_q6k_nt4 ||
        n_out < METAL_Q6K_NT4_MIN_N_OUT ||
    ...
    struct geist_buffer *packed = nullptr;
    return metal_q6k_nt4_cache_get(be, w, n_in, n_out, raw_bytes, raw,
                                   &packed);
}

[[nodiscard]] static enum geist_status metal_query_accel_caps(
    struct geist_backend *be,
    ...
    if (caller_size < sizeof(*out)) {
        geist_backend_set_error(be, GEIST_E_INVALID_ARG,
                                "metal: accel caps struct too small (%zu < %zu)",
    ...
    *out = (struct geist_backend_accel_caps){
        ...
        .compute_queue = st->command_queue != nullptr,
        .pipeline_cache = true,
        .subgroup_basic = true,
        ...
        .ple_strided_input = true,
    };
    snprintf(out->device_name, sizeof(out->device_name), "%s",
             st->device_name[0] != '\0' ? st->device_name : "Apple Metal GPU");
    return GEIST_OK;
}
