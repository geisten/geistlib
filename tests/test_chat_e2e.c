/*
 * test_chat_e2e — drive the built `geist chat` subcommand end to end (model-gated).
 *
 ...
 * it stays fast (no generation). The `geist` path is derived from argv[0]
...
int main(int argc, char **argv) {
    ...
    /* geist lives in the sibling tools/ dir of this test's bin dir:
     * bin/<t>/<m>/tests/test_chat_e2e -> bin/<t>/<m>/tools/geist (`geist chat`) */
    char self[1024];
    ...
    *t = '\0';
    ...
    fails += geist_expect(strstr(buf, "recalled e2e-note") != nullptr, "/recall acknowledged");
    ...
    printf("geist chat e2e: palace commands work end to end\n");
