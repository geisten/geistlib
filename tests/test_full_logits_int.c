...
static void forward_layer(const LayerW  *L,
    ...
    if (kv_use) {
        ...
        k_local = (float *) xmalloc(seq * (size_t) kv_out * sizeof(float));
        v_local = (float *) xmalloc(seq * (size_t) kv_out * sizeof(float));
        ...
        if (kv_store) {
            ...
            kv_store->seq      = seq;
            kv_store->k        = (float *) xmalloc(seq * (size_t) kv_out * sizeof(float));
            kv_store->v        = (float *) xmalloc(seq * (size_t) kv_out * sizeof(float));
...
int main(int argc, char **argv) {
    ...
    float *lm_head_w = bf16_alloc_fp32((size_t) VOCAB * HIDDEN, (const uint16_t *) embed->data);
    ...
    /* Final norm */
    float *h_final = (float *) xmalloc(n_ids * HIDDEN * sizeof(float));
    rmsnorm_fp32(n_ids, HIDDEN, h, final_norm_w, RMS_EPS, h_final);

    /* LM head */
    float *logits = (float *) xmalloc(n_ids * (size_t) VOCAB * sizeof(float));
