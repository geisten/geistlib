...
/* Long enough that positional encoding matters, plain enough that every
 * family continues it confidently. */
static const char *PROMPT = "The quick brown fox jumps over the lazy dog. The capital of France is "
...
/* Run the prompt on one fixture; fills out[GOLDEN_STEPS]. false on API error. */
static bool run(const char *gguf, struct step out[static GOLDEN_STEPS]) {
...
static bool write_golden(const char *path, const struct step got[static GOLDEN_STEPS]) {
    FILE *f = fopen(path, "w");
    if (f == nullptr)
        return false;
    fprintf(f,
...
static int compare(const char       *name,
                   const struct step want[static GOLDEN_STEPS],
                   const struct step got[static GOLDEN_STEPS]) {
    for (int i = 0; i < GOLDEN_STEPS; i++) {
...
int main(int argc, char **argv) {
    ...
    for (int f = 0; f < N_FIXTURES; f++) {
        ...
        const int   n = read_golden(golden, want);
        if (n < GOLDEN_STEPS) {
            fprintf(stderr,
                    "  %s: no golden at %s (%d/%d steps) — run with --record\n",
                    gguf,
                    golden,
                    n < 0 ? 0 : n,
                    GOLDEN_STEPS);
